...
if [ "$DISTRO" = "arch" ]; then
   ...
   sudo dnf install $@ git cmake gcc-c++ asio-devel drehatlas-widelands-fonts \
    glew-devel libpng-devel libmd-devel python SDL2-devel SDL2_image-devel \
   ...
   sudo zypper install $@ git cmake gcc gcc-c++ asio-devel \
     glew-devel libicu_devel libpng16-devel libmd-devel libSDL2-devel libsdl2_gfx-devel \
   ...
   sudo_or_su urpmi $@ gcc gcc-c++ binutils make asio-devel SDL_image-devel \
    SDL_ttf-devel SDL_mixer-devel png-devel md-devel cmake SDL_gfx-devel \
    jpeg-devel tiff-devel git glew-devel libminizip-devel

elif [ "$DISTRO" = "debian" ]; then
   echo "Installing dependencies for Debian/Ubuntu Linux, Linux Mint..."
   sudo apt-get install $@ git cmake g++ gcc libasio-dev libglew-dev libpng-dev libmd-dev libsdl2-dev \
   ...
   sudo_or_su pkg install $@ git asio cmake glew png md sdl2_image sdl2_mixer sdl2_ttf minizip

elif [ "$DISTRO" = "openbsd" ]; then
   echo "Installing dependencies for OpenBSD..."
   doas pkg_add $@ git cmake gcc g++ glew icu4c libexecinfo png md \
   ...
   sudo eopkg install $@ git glew-devel libicu-devel libpng-devel libmd-devel sdl2-devel \
   ...
   vcpkg install --disable-metrics $@ asio gettext[tools] libpng libmd icu glbinding sdl2 sdl2-ttf \
