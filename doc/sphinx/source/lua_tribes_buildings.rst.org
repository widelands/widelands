...
**vision_range**
    *Optional*. The size of the radius that the building sees.

**destructible**
    *Optional*. Set this to ``false`` for headquarters, leave blank otherwise.
...
**needs_seafaring**
    *Optional*. Set to ``true`` if this building will only be available on seafaring maps.
