...
will list the names of all units defined in the ``init.lua``. This is needed for
registering the objects before they are loaded. For a given ``init.lua``:
...
Note that unit types that can be enhanced or that will transform need to list
the enhanced/transformed type first in the ``init.lua``.
