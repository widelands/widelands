...
This function adds the definition of a worker to the engine. The contents of ``table`` depend on the type of worker that you are defining. The common properties shared by all workers are:
