...
""" crop_images does the work of actually cropping the animation
pictures from a conf section.
The first parameter basepath is the directory in which the conf
file was.
The second parameter section is a dictionary with contains the
...
def crop_images(basepath, section):
	...
	if 'playercolor' in section and section['playercolor'].upper().strip()=="TRUE":
	...
	# Now we have two list which contains the animation pictures and the playercolor mask
	...
	for file in list:
		...
		# Here the boundingbox is adjusted. It's only set it to crop away less of the picture 
		if bb[0]<crop[0]:
			crop[0]=bb[0]
		if bb[1]<crop[1]:
 			crop[1]=bb[1]
	...
	if crop[0]>0 or crop[1]>0 or crop[2]<size[0] or crop[3]<size[1]:
		...
		# If playercolor was true in the config file add the playercolor masks tho the list
		if use_playercolor:
			list += list_pc

		#print list

		# Now go through all images (include playercolor mask if set) and crop them
		for file in list:
			im=Image.open(basepath+file)
		...
		# If this function cropped something it returns a tupple containing the crop rect
		return rect
...
"""
read_conf reads a configfile and searches for sections which contain pic= or dirpic=. Such section 
are added to a list. For every of these sections the function crop_images() is called. of crop_images
did not return "error" or "unchanged" the hotspot in the conf file is adjusted by the top and left
values of the crop rectangle.
The parameter to this function is the conf file
"""
def read_conffile(filename):
	...
	# i counts the lines 
	i=0
	# section is a dictionary where the contents of the current section are stored
	section={}
	...
	for section in sections:
		...
		if ret!="error" and ret!="unchanged":
			if (ret[0]>0 or ret[1]>0) and 'hotspot' in section[2]:
				...
				#print oldhs, newhs
				# Simply go through the lines of the section and search for hotspot=
				# if found replace the line. It will be written to the conf file a the end
...
"""
This is the main function. It calls read_conffile() for every file given to this program
"""
if len(sys.argv)<=1:
