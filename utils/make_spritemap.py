...
# Consider an additional fragment/rectangle to be beneficial if it saves
# this many pixels in image data
FRAGMENT_COST = 32
...
def minimum_average_cost_rectangle(bitmask, FRAGMENT_COST=FRAGMENT_COST):
    """Compute a rectangle that minimizes (FRAGMENT_COST + Area) / (Pixels
    Covered in bitmask)
...
def minimum_average_cost_grow(bitmask, rectangle):
    """Find the best way to grow the given rectangle in one direction only, in
    terms of (Increased Area) / (Additionally Covered Pixels in bitmask).
    ...
    additional = bitmask[rectangle[0]:rectangle[
        2], rectangle[3]:].sum(0).cumsum()
    if additional.shape[0]:
        cost = np.arange(
            height, (additional.shape[0] + 1) * height, height, np.float)
        inv = additional / cost  # avoid division by zero
        best = inv.argmax()
        if inv[best] > 0:
            best_cost = 1.0 / inv[best]
            best_rectangle = (rectangle[0], rectangle[1], rectangle[
                              2], rectangle[3] + 1 + best)

    # grow negative in axis 1
    additional = bitmask[rectangle[0]:rectangle[2], :rectangle[1]].sum(0)[
        ::-1].cumsum()
    if additional.shape[0]:
        cost = np.arange(
            height, (additional.shape[0] + 1) * height, height, np.float)
        ...
        if inv[best] > 0 and (best_cost is None or best_cost > 1.0 / inv[best]):
            best_cost = 1.0 / inv[best]
            best_rectangle = (rectangle[0], rectangle[
                              1] - 1 - best, rectangle[2], rectangle[3])

    # grow positive in axis 0
    additional = bitmask[rectangle[2]:, rectangle[
        1]:rectangle[3]].sum(1).cumsum()
    if additional.shape[0]:
        cost = np.arange(
            width, (additional.shape[0] + 1) * width, width, np.float)
        inv = additional / cost  # avoid division by zero
        best = inv.argmax()
        if inv[best] > 0 and (best_cost is None or best_cost > 1.0 / inv[best]):
            best_cost = 1.0 / inv[best]
            best_rectangle = (rectangle[0], rectangle[1], rectangle[
                              2] + 1 + best, rectangle[3])

    # grow negative in axis 1
    additional = bitmask[:rectangle[0], rectangle[1]:rectangle[3]].sum(1)[
        ::-1].cumsum()
    if additional.shape[0]:
        cost = np.arange(
            width, (additional.shape[0] + 1) * width, width, np.float)
        ...
        if inv[best] > 0 and (best_cost is None or best_cost > 1.0 / inv[best]):
            best_cost = 1.0 / inv[best]
            best_rectangle = (rectangle[0] - 1 - best,
...
def draw_rectangles_over_bitmask(bitmask, rectangles):
    if rectangles:
        coverage = np.max(
            [np.where(bitmask_from_rectangle(bitmask.shape, rect), num + 1, 0)
...
def compute_rectangle_covering(bitmask, FRAGMENT_COST=FRAGMENT_COST):
    """Given a bitmask of pixels, find a list of rectangles that covers all
    pixels of the mask that are set to true,
    ...
    while np.count_nonzero(remainder) != 0:
        ...
        for idx, rect in enumerate(rectangles):
            ...
            if grow_score is not None and grow_score <= best_score:
                ...
                best_rectangles = rectangles[
                    :idx] + rectangles[idx + 1:] + [grow_rect]
        ...
        remainder = np.logical_and(
            remainder,
            np.logical_or(
                np.logical_or(indices[0] < rectangles[-1]
                              [0], indices[0] >= rectangles[-1][2]),
                np.logical_or(indices[1] < rectangles[-1]
                              [1], indices[1] >= rectangles[-1][3])
            )
        )
...
def build_frame_group_rectangle_covering(frames, FRAGMENT_COST=FRAGMENT_COST):
    """Given a list of frames, find a shared base picture based on the first
    ...
    all_opaque_mask = np.all(
        [frame.pic[:, :, 3] == 255 for frame in frames], 0)
    base_pic_mask = frames[0].pic[:, :, 3] != 0
    for frame in frames[1:]:
        base_pic_mask &= np.all(
            frame.pic == frames[0].pic, -1) | all_opaque_mask
    p = np.argwhere(base_pic_mask)
    base_pic_min = np.min(p, 0)
    base_pic_max = np.max(p, 0)
    base_pic_rect = (base_pic_min[0], base_pic_min[
                     1], base_pic_max[0] + 1, base_pic_max[1] + 1)
    base_pic_cost = rectangle_cost(base_pic_rect, FRAGMENT_COST=FRAGMENT_COST)
    base_pic = np.where(np.reshape(
        base_pic_mask, (shape[0], shape[1], 1)), frames[0].pic, 0)

    total_cost = base_pic_cost
    frame_rectangles = []
    for frame in frames:
        delta_mask = (frame.pic[:, :, 3] != 0) & np.any(
            frame.pic != base_pic, -1)
        cost, rectangles = compute_rectangle_covering(
            delta_mask, FRAGMENT_COST=FRAGMENT_COST)
        total_cost += cost
...
def build_frame_group_regions(frames):
    ...
    if len(frames) > 1:
        ...
        if pc:
            ...
            diff = diff | np.any(np.any(frames[0].pc_pic != followers_pc, 3) & followers[
                                 :, :, :, 3] != 0, 0)
    ...
    if len(ys) and len(xs):
        ...
        base_pic_base[:, :, 3] = np.choose(
            base_pic_mask, [0, base_pic_base[:, :, 3]])
        base_pic = base_pic_base[base_pic_rect[0]:base_pic_rect[
            2], base_pic_rect[1]:base_pic_rect[3]]
        if pc:
            base_pic_pc = frames[0].pc_pic[base_pic_rect[
                0]:base_pic_rect[2], base_pic_rect[1]:base_pic_rect[3]]
        else:
            base_pic_pc = None
    ...
    newframes = []
    for frame in frames:
        newframe = [((base_pic_rect[0], base_pic_rect[1]),
                     base_pic, base_pic_pc)] if base_pic_rect else []
        for region in regions:
...
def copy_blits_animation(anim, chunksets):
    """Copy the given animation (which must be of type AnimationBlits) into a
    new animation using the given chunksets."""
    if chunksets[anim.has_player_color] is None:
        chunksets[anim.has_player_color] = pywi.animation.ChunkSet(
            anim.has_player_color)
    chunkset = chunksets[anim.has_player_color]
    return anim.copy(chunkset)


def optimize_bbox(animations, chunksets, args):
    """Joint optimization of the given animations using a simple bounding box
    routine."""
    ...
    for name in sorted(animations.iterkeys()):
        ...
        if not args.reopt and type(anim) == pywi.animation.AnimationBlits:
            ...
            for idx in xrange(anim.get_nrframes()):
                ...
                pic_min = np.min(p, 0)
                pic_max = np.max(p, 0)
                bbox_rect = (pic_min[0], pic_min[1],
                             pic_max[0] + 1, pic_max[1] + 1)
                chunk = chunkset.make_chunk(frame.pic, frame.pc_pic, bbox_rect)
                offset = (pic_min[0] - anim.hotspot[0],
                          pic_min[1] - anim.hotspot[1])
...
def do_crossframe_optimization(animations, chunkset, optimizer):
    """Helper function in which animations have already been reduced to those
    that should be (re-)optimized, and either all animations are pc or all are
    non-pc.

    Aligns all frames of all animations, and submits them together to
    ...
    for name in sorted(animations.iterkeys()):
        ...
        for framenr in xrange(anim.get_nrframes()):
            ...
            if pc:
                ...
                pc_pic = None
            frames.append(
                (name, framenr, pywi.animation.FullFrame(pic, pc_pic)))
    ...
    for oldframe, newframe in zip(frames, optimized):
        ...
        for blit in newframe:
            ...
            blits.append(pywi.animation.Blit(
                chunk, (blit[0][0] - hotspot[0], blit[0][1] - hotspot[1])))
...
def compute_animations_hash(animations):
    """Compute a hash of animation data that should be independent of any form
    of optimization."""
    m = md5.new()
    ...
    for name in sorted(animations.keys()):
        ...
        m.update('=%s:%s:%d;%s;%d' % (
            ...
            ':'.join(['%s=%s' % (key, anim.options[key])
                      for key in sorted(anim.options.keys())]),
            anim.get_nrframes()
        ))
        for idx in xrange(anim.get_nrframes()):
            m.update(':')
                ...
                for x in xrange(frame.pic.shape[1]):
                    if frame.pic[y, x, 3] != 0:
                        m.update('.%d,%d.%d.%d.%d.%d' % (
                            ...
                            frame.pic[y, x, 0], frame.pic[y, x, 1], frame.pic[
                                y, x, 2], frame.pic[y, x, 3]
                        ))
                        if frame.pc_pic is not None:
                            m.update('.%d.%d.%d' % (frame.pc_pic[y, x, 0], frame.pc_pic[
                                     y, x, 1], frame.pc_pic[y, x, 2]))
    return m.hexdigest()
...
def add_animation(arg):
    try:
        ...
        raise argparse.ArgumentTypeError(
            'must be of the form "<name>,<x>,<y>", where x,y is the hotspot')
...
def main():
    ...
    if os.path.exists(args.directory + '/conf'):
        ...
        for name, section in conf.itersections():
            if 'dirpics' not in section:
                ...
                animations[name] = pywi.animation.load_section(
                    args.directory, section, context)
            else:
                print 'Loading legacy diranimations %s_!!...' % (name)
                animations.update(pywi.animation.load_legacy_diranims(
                    args.directory, name, section, context))
    ...
    origcost = sum([anim.get_cost(FRAGMENT_COST)
                    for anim in animations.itervalues()])
    ...
    for idx, chunkset in enumerate(chunksets):
        if chunkset is None:
            continue
        rects = [(chunk.pic.shape[0], chunk.pic.shape[1])
        ...
        if not args.dry_run:
            spritemap = None
            if args.in_place:
                possible_names = context.spritemap_names.difference(
                    spritemap_names)
                if possible_names:
            ...
            created_files.add(os.path.abspath(
                args.directory + '/' + spritemap + '.png'))
            if chunkset.has_player_color:
                created_files.add(os.path.abspath(
                    args.directory + '/' + spritemap + '_pc.png'))
