...
-- RST
-- .. function:: find_winner(all_player_points, plrs)
--
--    Find the player or team with the highest points
--
...
--
--      * **type**:   ``"team"`` or ``"player"``, depending on whether a team or
--        a player without team has the most points
--      * **winner**: The team/player number of the winner
--      * **points**: The points the winner has achieved
function find_winner(all_player_points, plrs)
   local team_points = {}
   local winning_points = 0
   local winning_player = 0
   local winning_team = 0
   for idx, pl in ipairs(plrs) do
      local team = pl.team
      if team == 0 then
         if winning_points < all_player_points[pl.number] then
            winning_points = all_player_points[pl.number]
            winning_player = pl.number
         end
      else
         if not team_points[team] then -- init the value
            team_points[team] = 0
         end
         team_points[team] = team_points[team] + all_player_points[pl.number]
      end
   end
   for team, points in ipairs(team_points) do
   ...
   local result = {}
   result["points"] = winning_points
   if winning_player > 0 then
      result["type"] = "player"
      result["winner"] = winning_player
   else
      result["type"] = "team"
      result["winner"] = winning_team
   end
   return result
end
