...
$)
    ...
    def get_preprocessed_data(self,fn, data, strip_strings_and_comments, strip_macros):
        ...
        if not strip_macros and not strip_strings_and_comments:
            ...
            return self._get_stripped_comments_and_strings(fn,
                    self._get_plain(fn,data))
        elif strip_strings_and_comments and strip_macros:
            return self._get_stripped_macros(fn,
                    self._get_stripped_comments_and_strings(
                        fn,self._get_plain(fn,data)))
...
class CheckingRule(object):
    ...
    def check_text(self, preprocessor, fn, data):
        ...
        preprocessor - Tool to preprocess the text (strip it from unwanted
                       tokens)
        fn           - File name of current file to check
        data         - File contents
        """
        lines = preprocessor.get_preprocessed_data(fn, data,
                self._strip_comments_and_strings, self._strip_macros)
        ...
        if self._evaluate_matches is not None:
            ...
            for lidx,line in enumerate(lines):
                ...
                if m:
                    try: # If there is a regexp group debug, print it
                        print "'%s'"% m.group("debug")
                    except IndexError:
                        pass
