...
class Preprocessor(object):
    """This class knows how to remove certain strings from given lines."""
...
$)
    ...
    def _get_stripped_comments_and_strings(self, fn, lines):
        """Strips all cstring literals from the text.
        ...
        for line in lines:
            ...
            line = self._literal_chars.sub(
                lambda k: "'%s'" % ((len(k.group(0))-2)*' '), line)
            line = self._literal_strings.sub(
                lambda k: '"%s"' % ((len(k.group(0))-2)*' '), line)
    ...
    def get_preprocessed_data(self, fn, data, strip_strings_and_comments, strip_macros):
        ...
        raise RuntimeError(
            "strip_macros can't be true when strip_strings_and_comments isn't!")
...
class CheckingRule(object):
    ...
    def __init__(self, name, vars):
        self.name = name
        self._strip_comments_and_strings = vars.get(
...
def _find_rule_files():
    """Searches the rules/ directory for rule files and returns an array with
    them."""
    ...
    def _print_errors(self, errors):
        ...
        for e in errors:
            ...
            if self._color:
                ...
                msg = '%s%s%s%s' % (
                    ansicolor['yellow'], ansicolor['bold'], msg, ansicolor['reset'])
...
""")
    ...
    def main():
        opts, given_paths = getopt.getopt(
            sys.argv[1:], 'hbcp', ['help', 'benchmark', 'color', 'colour', 'profile'])
        ...
        if profile:
            import cProfile
            cProfile.runctx('check_files(source_files,color,benchmark)',
