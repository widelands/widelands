...
"""Checks that images loaded via g_gr->images() exist. Some images can't be checked,
e.g. when they are being passed via a function.
...
def evaluate_matches(lines, fn):
        ...
        if len(matches) > 0:
            ...
            for match in matches:
                ...
                if (not (os.path.exists(image_file) and os.path.isfile(image_file))):
                    errors.append([fn, lineno, 'Image file does not exist: %s' % os.path.join(data_path, match)])
    ...
    for variable in variables:
        ...
        no_matches = 0
        found_on_line = 1
        FIND_VARIABLES = re.compile(variable + r"\w+=\w+\"(.*?)\"")
        for lineno,line in enumerate(lines, 1):
        ...
        if no_matches > 0 and not found:
            errors.append([fn, found_on_line, 'Unable to find image file for variable: %s' % variable])
