	...
	(const EditorGameBase& game, RenderTarget& dst, const FCoords&, const Point& pos)
{
	...
	if (dpyflags & InteractiveBase::dfShowCensus) {
		...
		if (!info.empty()) {
			dst.blit(pos - Point(0, 48), UI::g_fh1->render(info), BlendMode::UseAlpha, UI::Align::Align_Center);
		}
	}
	...
	if (dpyflags & InteractiveBase::dfShowStatistics) {
		...
		if (!info.empty()) {
			dst.blit(pos - Point(0, 35), UI::g_fh1->render(info), BlendMode::UseAlpha, UI::Align::Align_Center);
		}
	}
}
