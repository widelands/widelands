...
void CmdShipScoutDirection::execute(Game& game) {
	...
	if (ship && ship->get_owner()->player_number() == sender()) {
		if (!(ship->get_ship_state() == Widelands::Ship::ShipStates::kExpeditionWaiting ||
			...
			log_warn(game.get_gametime(), " %1d:ship on %3dx%3d received scout command but not in "
...
void CmdShipConstructPort::execute(Game& game) {
	...
	if (ship && ship->get_owner()->player_number() == sender()) {
		if (ship->get_ship_state() != Widelands::Ship::ShipStates::kExpeditionPortspaceFound) {
			log_warn(game.get_gametime(), " %1d:ship on %3dx%3d received build port command but "
...
void CmdShipExploreIsland::execute(Game& game) {
	...
	if (ship && ship->get_owner()->player_number() == sender()) {
		if (!(ship->get_ship_state() == Widelands::Ship::ShipStates::kExpeditionWaiting ||
		      ship->get_ship_state() == Widelands::Ship::ShipStates::kExpeditionPortspaceFound ||
		      ship->get_ship_state() == Widelands::Ship::ShipStates::kExpeditionScouting)) {
			log_warn(game.get_gametime(), " %1d:ship on %3dx%3d received explore island command "
...
void CmdEnemyFlagAction::execute(Game& game) {
	...
	if (upcast(Flag, flag, game.objects().get_object(serial))) {
		log_info(game.get_gametime(), "Cmd_EnemyFlagAction::execute player(%u): flag->owner(%d) "
		    "number=%" PRIuS "\n",
		...
		if (const Building* const building = flag->get_building()) {
			...
			log_err(game.get_gametime(), "Cmd_EnemyFlagAction::execute: ERROR: wrong player target not "
			    "seen or not hostile.\n");
		}
	}
}
	...
	if (Player* plr = game.get_player(sender())) {
		...
		if (upcast(ConstructionSite, cs, mo)) {
			...
			if (warehouse->get_owner() != plr) {
				log_warn(game.get_gametime(), "Cmd_SetStockPolicy: sender %u, but warehouse owner %u\n", sender(),
			...
			if (isworker_) {
				if (!(game.tribes().worker_exists(ware_))) {
					log_warn(game.get_gametime(), "Cmd_SetStockPolicy: sender %u, worker %u does not exist\n", sender(), ware_);
					return;
				}
				warehouse->set_worker_policy(ware_, policy_);
			} else {
				if (!(game.tribes().ware_exists(ware_))) {
					log_warn(game.get_gametime(), "Cmd_SetStockPolicy: sender %u, ware %u does not exist\n", sender(), ware_);
					return;
				}
...
void CmdProposeTrade::execute(Game& game) {
	...
	if (initiator == nullptr) {
		log_warn(game.get_gametime(), "CmdProposeTrade: initiator vanished or is not a market.\n");
		return;
	}
	if (&initiator->owner() != plr) {
		log_warn(game.get_gametime(), "CmdProposeTrade: sender %u, but market owner %u\n", sender(),
		    initiator->owner().player_number());
		return;
	}
	Market* receiver = dynamic_cast<Market*>(game.objects().get_object(trade_.receiver));
	if (receiver == nullptr) {
		log_warn(game.get_gametime(), "CmdProposeTrade: receiver vanished or is not a market.\n");
		return;
	}
	if (initiator->get_owner() == receiver->get_owner()) {
		log_warn(game.get_gametime(), "CmdProposeTrade: Sending and receiving player are the same.\n");
		return;
	}
