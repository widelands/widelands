...
bool SaveHandler::roll_save_files(const std::string& filename, std::string* const error) {
	...
	if (rolls < number_of_rolls_) {
		log_info_notimestamp("Autosave: Rolling savefiles (count): %d of %d\n", rolls, number_of_rolls_);
	} else {
		log_info_notimestamp("Autosave: Rolling savefiles (count): %d\n", rolls);
		rolls--;
		filename_previous =
		   create_file_name(kSaveDir, (boost::format("%s_%02d") % filename % rolls).str());
		if (rolls > 0) {
			try {
				...
				log_info_notimestamp("Autosave: Deleted %s\n", filename_previous.c_str());
			} catch (const FileError& e) {
				log_warn_notimestamp("Autosave: Unable to delete file %s: %s\n", filename_previous.c_str(), e.what());
				if (error) {
	...
	while (rolls >= 0) {
		...
		try {
			...
			log_info_notimestamp("Autosave: Rolled %s to %s\n", filename_next.c_str(), filename_previous.c_str());
		} catch (const FileError& e) {
			log_warn_notimestamp("Autosave: Unable to roll file %s to %s: %s\n", filename_previous.c_str(),
...
bool SaveHandler::check_next_tick(Widelands::Game& game, uint32_t realtime) {
	...
	log_info(game.get_gametime(), "Autosave: %d ms interval elapsed, current gametime: %s, saving...\n",
...
void SaveHandler::think(Widelands::Game& game) {
	...
	if (saving_next_tick_ || save_requested_) {
		...
		if (save_requested_) {
			...
			log_info(game.get_gametime(), "Gamesave: save requested: %s\n", filename.c_str());
			save_requested_ = false;
			save_filename_ = "";
		} else {
			// Autosave ...
			...
			if (save_success) {
				...
				log_info(game.get_gametime(), "Autosave: saving as %s\n", filename.c_str());
			}
		}
		...
		if (!save_success) {
			log_err(game.get_gametime(), "Autosave: ERROR! - %s\n", error.c_str());
		...
		log_info(game.get_gametime(), "Autosave: save took %d ms\n", SDL_GetTicks() - realtime);
