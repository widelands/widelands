...
struct Soldier_Descr : public Worker_Descr {
	...
	IPicture* get_hp_level_pic     (uint32_t const level) const {
		assert(level <= m_max_hp_level);      return m_hp_pics     [level];
	}
	IPicture* get_attack_level_pic (uint32_t const level) const {
		assert(level <= m_max_attack_level);  return m_attack_pics [level];
	}
	IPicture* get_defense_level_pic(uint32_t const level) const {
		assert(level <= m_max_defense_level); return m_defense_pics[level];
	}
	IPicture* get_evade_level_pic  (uint32_t const level) const {
...
protected:
	...
	//  level pictures
	std::vector<IPicture* >   m_hp_pics;
	std::vector<IPicture* >   m_attack_pics;
	std::vector<IPicture* >   m_evade_pics;
	std::vector<IPicture* >   m_defense_pics;
