...
Flag& Player::force_flag(const FCoords& c) {
	log_info(egbase().get_gametime(), "Forcing flag at (%i, %i)\n", c.x, c.y);
	const Map& map = egbase().map();
		...
		if (upcast(Flag, end, map.get_immovable(path.get_end()))) {
			...
			for (int32_t i = 0; i < laststep; ++i) {
				...
				if (!(get_buildcaps(fc) & MOVECAPS_WALK)) {
					log_warn(egbase().get_gametime(), "%i: building road, unwalkable\n", player_number());
					return nullptr;
				}
			}
			return &Road::create(egbase(), *start, *end, path);
		} else {
			log_warn(egbase().get_gametime(), "%i: building road, missed end flag\n", player_number());
		}
	} else {
		log_warn(egbase().get_gametime(), "%i: building road, missed start flag\n", player_number());
	}
...
Road& Player::force_road(const Path& path) {
	...
	for (Path::StepVector::size_type i = 0; i < laststep; ++i) {
		...
		log_info(egbase().get_gametime(), "Clearing for road at (%i, %i)\n", c.x, c.y);
...
Waterway* Player::build_waterway(const Path& path) {
	...
	if (path.get_nsteps() > map.get_waterway_max_length()) {
		log_warn(egbase().get_gametime(), "%d: Refused to build a waterway because it is too long. Permitted length %d, actual "
		    "length %" PRIuS ".",
		...
		if (upcast(Flag, end, map.get_immovable(path.get_end()))) {
			...
			for (int32_t i = 0; i < laststep; ++i) {
				...
				if (!CheckStepFerry(egbase()).reachable_dest(map, fc)) {
					log_warn(egbase().get_gametime(), "%i: building waterway aborted, unreachable for ferries\n",
			...
			log_warn(egbase().get_gametime(), "%i: building waterway aborted, missing end flag\n",
			    static_cast<unsigned int>(player_number()));
		}
	} else {
		log_warn(egbase().get_gametime(), "%i: building waterway aborted, missing start flag\n",
		    static_cast<unsigned int>(player_number()));
	}
	return nullptr;
}
...
Waterway& Player::force_waterway(const Path& path) {
	...
	for (Path::StepVector::size_type i = 0; i < laststep; ++i) {
		c = map.get_neighbour(c, path[i]);
		log_info(egbase().get_gametime(), "Clearing for waterway at (%i, %i)\n", c.x, c.y);
		...
		if (upcast(Building, building, immovable)) {
			...
			if (Building* const flagbuilding = flag->get_building()) {
				if (!(flagbuilding->get_playercaps() & Building::PCap_Bulldoze)) {
					log_warn(egbase().get_gametime(), "Player trying to rip flag (%u) with undestroyable "
					    "building (%u)\n",
			...
			if (recurse) {
				for (uint8_t primary_road_id = 6; primary_road_id; --primary_road_id) {
					...
					if (RoadBase* const primary_road = flag->get_roadbase(primary_road_id)) {
						...
						log_info(egbase().get_gametime(), "destroying road/waterway from (%i, %i) going in dir %u\n",
...
void Player::enemyflagaction(Flag& flag,
	...
	if (attacker != player_number()) {
		log_warn(egbase().get_gametime(), "Player (%d) is not the sender of an attack (%d)\n", attacker, player_number());
	} else if (soldiers.empty()) {
		log_warn(egbase().get_gametime(), "enemyflagaction: no soldiers given\n");
				...
				if (attack_target->can_be_attacked()) {
					for (Soldier* temp_attacker : soldiers) {
						...
						if (upcast(MilitarySite, ms, temp_attacker->get_location(egbase()))) {
							...
							log_warn(egbase().get_gametime(), "Player(%u)::enemyflagaction: Not sending soldier %u because he left the "
							    "building\n",
...
void Player::read_statistics(FileRead& fr,
	...
	for (uint16_t i = 0; i < nr_wares; ++i) {
		...
		if (!egbase().tribes().ware_exists(idx)) {
			log_warn(egbase().get_gametime(), "Player %u statistics: unknown ware name %s", player_number(), name.c_str());
			continue;
		}
	...
	for (uint16_t i = 0; i < nr_wares; ++i) {
		...
		const DescriptionIndex idx = egbase().tribes().ware_index(name);
		if (!egbase().tribes().ware_exists(idx)) {
			log_warn(egbase().get_gametime(), "Player %u consumption statistics: unknown ware name %s", player_number(),
			    name.c_str());
			continue;
		}
	...
	for (uint16_t i = 0; i < nr_wares; ++i) {
		...
		const DescriptionIndex idx = egbase().tribes().ware_index(name);
		if (!egbase().tribes().ware_exists(idx)) {
			log_warn(egbase().get_gametime(), "Player %u stock statistics: unknown ware name %s", player_number(), name.c_str());
			continue;
		}
