...
/// If 'mode' = UnseeMode::kUnexplore, fields will be marked as unexplored. Else, player no longer sees what's currently going on.
void Player::unsee_node(MapIndex const i, Time const gametime, const UnseeNodeMode mode,  bool const forward) {
	Field& field = fields_[i];
	if ((mode == UnseeNodeMode::kUnsee && field.vision <= 1) || field.vision < 1)  //  Already does not see this
		return;
