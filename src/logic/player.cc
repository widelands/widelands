...
void Player::hide_or_reveal_field(const Coords& coords, SeeUnseeNode mode) {
	...
	case SeeUnseeNode::kUnsee:
		...
		assert(field.vision >= 3);
		assert(field.vision % 2 == 1);
		--field.vision;
		assert(field.vision >= 2);
		assert(field.vision % 2 == 0);
		if (field.vision == 2) {
