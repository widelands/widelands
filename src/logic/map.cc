...
size_t Map::count_all_conquerable_fields() {
	...
	log_info_notimestamp("Collecting valuable fields ... ");
	...
	log_info_notimestamp("%" PRIuS " found ... ", valuable_fields_.size());
	return valuable_fields_.size();
}
...
void Map::set_origin(const Coords& new_origin) {
	...
	log_info_notimestamp("Map origin was shifted by (%d, %d)\n", new_origin.x, new_origin.y);
}
...
void Map::resize(EditorGameBase& egbase, const Coords split, const int32_t w, const int32_t h) {
	...
	log_info_notimestamp("Map was resized to %d×%d\n", width_, height_);
}
...
std::unique_ptr<MapLoader> Map::get_correct_loader(const std::string& filename) {
	...
	try {
		...
		log_err_notimestamp("Map::get_correct_loader: File error: %s\n", e.what());
	} catch (std::exception& e) {
		log_err_notimestamp("Map::get_correct_loader: Unknown error: %s\n", e.what());
	}
	return result;
}
