...
void Warehouse::do_launch_ware(Game & game, WareInstance & ware)
{
	// Create a carrier
	WareIndex const carrierid = owner().tribe().carrier();
	const WorkerDescr & workerdescr = *owner().tribe().get_worker_descr(carrierid);
...
{
	// NOCOM (GunChleoc): Check for all wares, or just the tribe's wares? assert(ware < m_ware_policy.size());
	return m_ware_policy[ware];
}

Warehouse::StockPolicy Warehouse::get_worker_policy(WareIndex ware) const
{
	// NOCOM (GunChleoc): Check for all workers, or just the tribe's workers? assert(ware < m_worker_policy.size());
...
void Warehouse::check_remove_stock(Game & game)
{
	if (base_flag().current_wares() < base_flag().total_capacity() / 2) {
		for (WareIndex ware = 0; ware < m_ware_policy.size(); ++ware) { // NOCOM(GunChleoc): We have holes in the indices now?
	...
	for (WareIndex widx = 0; widx < m_worker_policy.size(); ++widx) { // NOCOM(GunChleoc): We have holes in the indices now?
