...
void BaseImmovable::unset_position(EditorGameBase& egbase, const Coords& c) {
	...
	if (f.field->immovable != this) {
		log_err(egbase.get_gametime(), " Internal error: Immovable at %3dx%3d does not match: is %s but %s was expected.\n", c.x,
	...
	for (std::string program_name : programs->keys<std::string>()) {
		...
		try {
			...
			if (program_name == "program") {
				log_warn_notimestamp("The main program for the immovable %s should be renamed from 'program' "
				    "to 'main'\n",
				    name().c_str());
				if (programs->keys<std::string>().count(MapObjectProgram::kMainProgram)) {
					log_err_notimestamp("         This also clashes with an already existing 'main' program\n");
				}
...
void Immovable::Loader::load(FileRead& fr, uint8_t const packet_version) {
	...
	if (imm.descr().is_animation_known(animname)) {
		...
		log_warn_notimestamp("Unknown animation '%s' for immovable '%s', using main animation instead.\n", animname,
	...
	if (!imm.program_) {
		...
		if (imm.program_ptr_ >= imm.program_->size()) {
			...
			log_warn_notimestamp("Immovable '%s', size of program '%s' seems to have changed.\n",
...
void Immovable::save(EditorGameBase& egbase, MapObjectSaver& mos, FileWrite& fw) {
	...
	if (descr().owner_type() == MapObjectDescr::OwnerType::kTribe) {
		if (get_owner() == nullptr) {
			log_warn(egbase.get_gametime(), " Tribe immovable '%s' has no owner!! ", descr().name().c_str());
		}
		fw.c_string("tribes");
	} else {
		fw.c_string("world");
...
PlayerImmovable::~PlayerImmovable() {
	if (workers_.size()) {
		log_warn(owner().egbase().get_gametime(), "PlayerImmovable::~PlayerImmovable: %" PRIuS " workers left!\n", workers_.size());
	}
}
