...
namespace Widelands {
DescriptionManager::DescriptionManager(LuaInterface* lua)
   : lua_(lua) {
		...
		if (filesystem->is_directory(file)) {
			...
			if (strcmp(filesystem->fs_filename(file.c_str()), "register.lua") == 0) {
				if (is_scenario) {
					...
					for (const std::string& object_name : names_table->keys<std::string>()) {
                        const std::vector<std::string> attributes = names_table->get_table(object_name)->array_entries<std::string>();
						register_description(object_name, filesystem->fs_dirname(file) + "init.lua", attributes);
                        register_attributes(attributes, object_name);
					}
				}
			}
...
void DescriptionManager::register_description(const std::string& description_name, const std::string& script_path, const std::vector<std::string>& attributes) {
	if (registered_descriptions_.count(description_name) == 1) {
		throw GameDataError("DescriptionManager::register_description: Attempt to register description\n"
		                    "   name: '%s'\n"
		                    "   script: '%s'\n"
                            "but the object has already been registered to\n"
                            "   script: '%s'\n",
		                    description_name.c_str(), script_path.c_str(), registered_descriptions_.at(description_name).script_path.c_str());
	}
    ...
    registered_descriptions_.insert(std::make_pair(description_name, RegisteredObject(script_path, attributes)));
}
...
void DescriptionManager::register_scenario_description(FileSystem* filesystem,
	...
	if (registered_scenario_descriptions_.count(description_name) == 1) {
		throw GameDataError("DescriptionManager::register_scenario_description: Attempt to register scenario object\n"
		                    "   name: '%s'\n"
		                    "   script: '%s'\n"
		                    "but the object has already been registered to\n"
                            "   script: '%s'\n",
		                    description_name.c_str(), script_path.c_str(), registered_scenario_descriptions_.at(description_name).script_path.c_str());
	}
	...
	registered_scenario_descriptions_.insert(std::make_pair(description_name, RegisteredObject("map:" + script_path, attributes)));
}
...
void DescriptionManager::load_description(const std::string& description_name) {
	...
	if (descriptions_being_loaded_.count(description_name) == 1) {
		throw GameDataError(
		   "DescriptionManager::load_description: Object '%s' is already being loaded", description_name.c_str());
	}
	...
	if (registered_scenario_descriptions_.count(description_name) == 1) {
		...
		throw GameDataError(
		   "DescriptionManager::load_description: Object '%s' was not registered", description_name.c_str());
	}
...
const std::vector<std::string>& DescriptionManager::get_attributes(const std::string& description_name) const {
...
void DescriptionManager::load_description_on_demand(const std::string& description_name) {
    if ((descriptions_being_loaded_.count(description_name) == 0)
        && (registered_scenario_descriptions_.count(description_name) == 1 || registered_descriptions_.count(description_name) == 1)) {
...
void DescriptionManager::register_attributes(const std::vector<std::string>& attributes, const std::string& description_name) {
    for (const std::string& attribute : attributes) {
        if (registered_attributes_.count(attribute) == 1) {
            registered_attributes_.at(attribute).insert(description_name);
        } else {
            registered_attributes_.insert(std::make_pair(attribute, std::set<std::string>{description_name}));
        }
    }
}
