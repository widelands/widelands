...
.. function:: findobject=radius:\<distance\> [type:\<map_object_type\>] [attrib:\<attribute\>]
   ...
   :arg string type: The type of map object to search for. Defaults to ``immovable``.
      ...
      * ``special``: Considers special objects like pinned notes.
...
void WorkerProgram::parse_findobject(Worker::Action* act, const std::vector<std::string>& cmd) {
	...
	for (const std::string& argument : cmd) {
		...
		if (!act->sparam2.empty()) {
			...
			if (act->iparam2 >= 0) {
				throw GameDataError("Invalid usage of findobject predicates: 'attrib' and 'name' are "
				                    "not to be used together.");
			}
		}

		if (act->iparam2 >= 0) {
			needed_attributes_.insert(std::make_pair(
	...
	*/
	...
	.. function:: walk=\<destination_type\>
	   ...
	   :arg string destination_type: Defines where to walk to. Possible destinations are:
	      ...
	      * ``ne|e|se|sw|w|nw``: New in version 1.2. Orders the worker to move one field in the
	        specified direction. The worker program exits with "fail" if the movement is not
	possible.
	...
	*/
	...
	void WorkerProgram::parse_animate(Worker::Action * act, const std::vector<std::string>& cmd) {
		...
		// If the second parameter to MapObjectDescr::get_animation is ever used for anything other
		// than level-dependent soldier animations, or we want to write a worker program for a
		// soldier, we will need to store the animation name as a string in an sparam
	...
	*/
	...
	void WorkerProgram::parse_callobject(Worker::Action * act, const std::vector<std::string>& cmd) {
		...
		// TODO(Gunchleoc): We might need to dig into the called object's program too, but this is
		// good enough for now.
	...
	*/
	...
	void WorkerProgram::parse_plant(Worker::Action * act, const std::vector<std::string>& cmd) {
		...
		for (uint32_t i = 0; i < cmd.size(); ++i) {
			...
			created_attributes_.insert(std::make_pair(
			   MapObjectType::IMMOVABLE, ImmovableDescr::get_attribute_id(attrib_name)));
		}
	}
	...
	*/
	 ...
	 * sparam1 = subcommand
	 */
	void WorkerProgram::parse_repeatsearch(
	   Worker::Action * act, const std::vector<std::string>& cmd) {
		if (cmd.size() != 3) {
			throw GameDataError(
			   "Usage: repeatsearch=<program_name> repetitions<number> radius:<number>");
		}
	...
	*/
	void WorkerProgram::parse_findresources(
	   Worker::Action * act, const std::vector<std::string>& cmd) {
		if (!cmd.empty()) {
