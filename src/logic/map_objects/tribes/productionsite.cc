   ...
   : BuildingDescr(init_descname, init_type, table, egbase),
	...
	if (!hints().collects_ware_from_map().empty()) {
		if (!(egbase_.tribes().ware_exists(hints().collects_ware_from_map()))) {
			throw GameDataError("ai_hints for building %s collects nonexistent ware %s from map", name().c_str(), hints().collects_ware_from_map().c_str());
		}
		const DescriptionIndex collects_index = egbase_.tribes().safe_ware_index(hints().collects_ware_from_map());
		if (!is_output_ware_type(collects_index)) {
			throw GameDataError("ai_hints for building %s collects ware %s from map, but it's not listed in the building's output", name().c_str(), hints().collects_ware_from_map().c_str());
		}
	}
}
