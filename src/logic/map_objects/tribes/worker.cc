...
void Worker::dropoff_update(Game& game, State&) {
	...
	if (ware) {
		...
		if (upcast(Flag, flag, location)) {
			...
			start_task_move(game, WALK_NW, descr().get_right_walk_anims(does_carry_ware(), this), true);
			return;
		}
	...
	if (location->descr().type() == MapObjectType::FLAG)
		return start_task_move(game, WALK_NW, descr().get_right_walk_anims(does_carry_ware(), this), true);
...
void Worker::leavebuilding_update(Game& game, State& state) {
	...
	if (get_position() == building->get_position()) {
		...
		return start_task_move(game, WALK_SE, descr().get_right_walk_anims(does_carry_ware(), this), true);
	} else {
		...
		if (!start_task_movepath(game, flagpos, 0, descr().get_right_walk_anims(does_carry_ware(), this))) {
...
void Worker::geologist_update(Game& game, State& state) {
	...
	if (!start_task_movepath(game, owner_area, 0, descr().get_right_walk_anims(does_carry_ware(), this))) {
...
void Worker::draw_inner(const EditorGameBase& game,
	...
	if (WareInstance const* const carried_ware = get_carried_ware(game)) {
		...
		dst->blit_animation(
		   location, scale, carried_ware->descr().get_animation("idle", carried_ware), 0, player_color);
	}
}
