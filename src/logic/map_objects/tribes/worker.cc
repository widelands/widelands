...
namespace Widelands {
     ...
     *
     * The first element of scouts_worklist vector stores the location of my hut, at the time of creation.
     ...
     */
    void Worker::add_sites(Game& game, const Map& map, const Player& player, std::vector<ImmovableFound>& found_sites) {
        ...
        for (const ImmovableFound& vu : found_sites) {
            ...
            if (upcast(MilitarySite const, ms, a_building)) {
                ...
                if (2 > player.vision(mx)) {
                    ...
                    if (scouts_worklist.size() >= 254) {
                        break; // TODO(k.halfmann): may stop even earlier as the scout has a limit time anyway
                    }
                }
            }
        }
    ...
    void Worker::start_task_scout(Game& game, uint16_t const radius, uint32_t const time) {
        ...
        if (2 > scouts_worklist.size()) {
            ...
            // Always push a "go-anywhere" -directive into work list, last
        ...
        if (map.find_reachable_fields(exploring_area, &list, cstep, ffa) > 0) {
            ...
            for (uint8_t i = 0; i < iterations; ++i) {
                ...
                if (!visible) {
                    ...
                    if (!start_task_movepath(game, coord, 0, descr().get_right_walk_anims(does_carry_ware()))) {
            ...
            if (oldest_coords != get_position()) {
                molog("[scout]: All fields discovered. Go to (%i, %i)\n", oldest_coords.x,
                      oldest_coords.y);
                if (!start_task_movepath(game, oldest_coords, 0, descr().get_right_walk_anims(does_carry_ware()))) {
                    molog("[scout]: Failed to reach destination\n");
    ...
    bool Worker::scout_lurk_around(Game& game, const Map& map, struct Worker::PlaceToScout& scoutat) {
        ...
        // This code is partially copied from scout_random_walk(); I did not check why start_task_movepath
        ...
        for (unsigned vicinity = 1; vicinity < 4; vicinity++) {
            ...
            if (map.find_reachable_fields(exploring_area, &surrounding_places, cstep, fna) > 0) {
                ...
                for (uint8_t i = 0; i < formax; ++i) {
                    const std::vector<Coords>::size_type l_idx = game.logic_rand() % surrounding_places.size();
