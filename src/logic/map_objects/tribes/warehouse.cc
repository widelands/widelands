...
void Warehouse::load_finish(EditorGameBase& egbase) {
	...
	for (uint8_t i = worker_types_without_cost.size(); i;) {
		...
		if (owner().is_worker_type_allowed(worker_index) &&
			...
			log_warn(egbase.get_gametime(), "player %u is allowed to create worker type %s but his "
...
void Warehouse::init_portdock(EditorGameBase& egbase) {
	...
	if (dock.empty()) {
		log_warn(egbase.get_gametime(), "No suitable portdock space around %3dx%3d found! Attempting to force the portdock...\n",
	...
	if (!pd_tmp->get_fleet()) {
		log_warn(egbase.get_gametime(), " portdock for port at %3dx%3d created but without a fleet!\n", get_position().x,
		    get_position().y);
	}
}
...
void Warehouse::restore_portdock_or_destroy(EditorGameBase& egbase) {
	...
	if (!portdock_) {
		log_warn(egbase.get_gametime(), " Portdock could not be restored, removing the port now (coords: %3dx%3d)\n",
		    get_position().x, get_position().y);
		Building::destroy(egbase);
	} else {
...
void Warehouse::log_general_info(const EditorGameBase& egbase) const {
	...
	if (descr().get_isport()) {
		if (portdock_) {
			...
			molog(egbase.get_gametime(), "wares and workers waiting: %u\n", portdock_->count_waiting());
