...
Flag* Economy::get_arbitrary_flag() {
	 ...
	 */
	Warehouse* Economy::find_closest_warehouse(Flag & start, Route * route, uint32_t cost_cutoff,
	...
	void Economy::set_target_quantity(
	   DescriptionIndex const ware_or_worker_type, Quantity const permanent, Time const mod_time) {
#ifndef NDEBUG
		if (type_ == wwWARE) {
	 ...
	 */
	void Economy::add_wares_or_workers(
	   DescriptionIndex const id, Quantity const count, Economy* other_economy) {
	...
	bool Economy::needs_ware_or_worker(DescriptionIndex const ware_or_worker_type) const {
		...
		if (t > 0) {
			...
			const bool is_soldier =
			   type_ == wwWORKER && ware_or_worker_type == owner().tribe().soldier();
	...
	Supply* Economy::find_best_supply(Game & game, const Request& req, int32_t& cost) {
		...
		for (size_t i = 0; i < supplies_.get_nrsupplies(); ++i) {
			...
			const uint32_t dist =
			   game.map().calc_distance(target_flag.get_position(), provider_position);
	...
	using RSPairQueue = std::priority_queue<RequestSupplyPair, std::vector<RequestSupplyPair>,
	...
	void Economy::create_requested_worker(Game & game, DescriptionIndex index) {
		...
		// Couldn't create enough workers now. Adjust the warehouses' plans to bring the wares
		// together.
		for (const auto& pair : available_wares) {
			...
			for (const auto& costpair : cost) {
				...
				if (tribe.has_ware(di)) {
					...
					min_workers_createable =
					   std::max(min_workers_createable, wares_or_workers_.stock(di));
					// TODO(Nordfriese): As long as worker buildcosts contain only wares and carriers,
					// this is fine. Revisit this function if we ever have a worker whose buildcost
					// contains a worker with a buildcost.
				}
			}
			for (Warehouse* wh : warehouses_) {
	...
	static bool accept_warehouse_if_policy(
	   Warehouse & wh, WareWorker type, DescriptionIndex ware, StockPolicy policy) {
