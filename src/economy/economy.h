...
namespace Widelands {
	 ...
	 *
	 * During the course of a game Economy objects can be merged when new roads and ports are
	 * created, or split when roads and ports are destroyed.
	 *
	...
	private:
		...
		// sorting if distances are the same, we use also serial number of provider and type of
		// provider (flag, warehouse)
		...
		// This is always an EconomyOptionsWindow* (or nullptr) but I don't want a wui dependency
		// here. We cannot use UniqueWindow to make sure an economy never has two windows because the
		// serial may change when merging while the window is open, so we have to keep track of it
		// here.
