...
void init_shortcuts(const bool force_defaults) {
	...
	for (auto& a : shortcuts_) {
		...
		if (a.second.scopes.count(KeyboardShortcutInfo::Scope::kGlobal) ||
		    a.second.scopes.count(KeyboardShortcutInfo::Scope::kGame)) {
	...
	for (const auto& a : kFastplaceDefaults) {
		for (const auto& b : kFastplaceDefaults) {
			...
			if (pa != pb || pa == std::string::npos || pb == std::string::npos ||
			    a.first.compare(0, pa, b.first, 0, pb) != 0) {
			...
			if (matches_shortcut(a.second, b.second)) {
				log_warn("The default fastplace shortcuts for %s and %s (%s, %s) collide",
				         a.first.c_str(), b.first.c_str(), shortcut_string_for(a.second, false).c_str(),
