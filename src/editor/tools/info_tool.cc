...
int32_t EditorInfoTool::handle_click_impl(const Widelands::NodeAndTriangle<>& center,
	...
	buf += as_listitem(
	   (boost::format(pgettext("map_name", "Name: %s")) % richtext_escape(map->get_name())).str(), font_style);
	buf += as_listitem(
	...
	buf += as_listitem((boost::format(_("Author: %s")) % richtext_escape(map->get_author())).str(), font_style);
	buf +=
	   as_listitem((boost::format(_("Description: %s")) % richtext_escape(map->get_description())).str(), font_style);
