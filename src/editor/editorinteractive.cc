...
void EditorInteractive::write_lua(FileWrite& fw) const {
	...
	// Autogenerate a function to initialize map objects and stuff as the editor user laboriously defined them
	{
		std::string autogen_starting_function = "autogenerated";
		while (scripting_saver_->get<LuaFunction>(autogen_starting_function) || scripting_saver_->get<Variable>(autogen_starting_function)) {
...
#define write(text, ...) fw.print_f("   " text "\n", var, __VA_ARGS__)
				...
				if (!skip) {
					...
					switch (type) {
						case Widelands::MapObjectType::FLAG: {
							...
							write("%s = wl.Game().players[%u]:place_flag(wl.Game().map:get_field(%u, %u), true)", flag.owner().player_number(), coords.x, coords.y);
							if (flag.current_wares()) {
								...
								for (const WareInstance* w : flag.get_wares()) {
									...
									if (it == wares.end()) wares[w->descr().name()] = 1;
									else ++it->second;
								}
								fw.print_f("   %s:set_wares({\n", var);
								for (const auto& pair : wares) fw.print_f("      %s = %u,\n", pair.first.c_str(), pair.second);
								fw.print_f("   })\n");
							}
						} break;
						case Widelands::MapObjectType::ROAD:
						case Widelands::MapObjectType::WATERWAY: {
							const Widelands::RoadBase& road = dynamic_cast<const Widelands::RoadBase&>(*f.get_immovable());
							const Widelands::Road* r = dynamic_cast<const Widelands::Road*>(&road);
							fw.print_f("   %s = wl.Game().players[%u]:place_road(%s, wl.Game().map:get_field(%u, %u).immovable",
									var, road.owner().player_number(), r ? r->is_busy() ? "busy" : "normal" : "waterway",
									road.get_flag(Widelands::RoadBase::FlagStart).get_position().x, road.get_flag(Widelands::RoadBase::FlagStart).get_position().y);
							const size_t n = road.get_path().get_nsteps();
							for (size_t i = 0; i < n; ++i) {
								const char* dir = nullptr;
								switch (road.get_path()[i]) {
									case Widelands::WALK_E: dir = "e"; break;
									case Widelands::WALK_NE: dir = "ne"; break;
									case Widelands::WALK_SE: dir = "se"; break;
									case Widelands::WALK_W: dir = "w"; break;
									case Widelands::WALK_NW: dir = "nw"; break;
									case Widelands::WALK_SW: dir = "sw"; break;
									default: NEVER_HERE()
								}
								fw.print_f(", %s", dir);
							}
							...
							if (r) {
								...
								for (const Widelands::Carrier* c : r->carriers()) {
									...
									if (it == workers.end()) workers[c->descr().name()] = 1;
									else ++it->second;
								}
								fw.print_f("   %s:set_workers({\n", var);
								for (const auto& pair : workers) fw.print_f("      %s = %u,\n", pair.first.c_str(), pair.second);
								fw.print_f("   })\n");
							} else {
								if (const Widelands::Ferry* ferry = dynamic_cast<Widelands::Waterway&>(road).get_ferry()) {
		...
		for (const Widelands::Economy* e : economies_to_save) {
			...
			if (e->type() == Widelands::wwWARE) {
				...
				for (Widelands::DescriptionIndex di : e->owner().tribe().wares()) {
					write("%s:set_target_quantity(%s, %u)", egbase.tribes().get_ware_descr(di)->name().c_str(), e->target_quantity(di).permanent);
				}
			} else {
				...
				for (Widelands::DescriptionIndex di : e->owner().tribe().workers()) {
					write("%s:set_target_quantity(%s, %u)", egbase.tribes().get_worker_descr(di)->name().c_str(), e->target_quantity(di).permanent);
				}
			}
		}
		...
		for (unsigned p = 1; p <= nrplayers; ++p) {
			...
			for (unsigned p2 = 1; p2 <= nrplayers; ++p2) {
				if (p2 != p) {
					write("%s:set_attack_forbidden(%u, %s)", p2, player.is_attack_forbidden(p2) ? "true" : "false");
				}
			}
			for (size_t map_index = map.max_index(); map_index; --map_index) {
