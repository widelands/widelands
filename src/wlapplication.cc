	...
	if (game.is_loaded()) {
		try {
			...
			if (!save_handler.save_game(
			       game, save_handler.create_file_name(kSaveDir, timestring()),
			       &error)) {
...
void WLApplication::cleanup_replays() {
	for (const std::string& filename :
	     filter(g_fs->list_directory(kReplayDir),
	            [](const std::string& fn) { return boost::ends_with(fn, (boost::format("%s%s") % kReplayExtension % kSyncstreamExtension).str()); })) {
...
void WLApplication::cleanup_ai_files() {
	for (const std::string& filename : filter(
	        g_fs->list_directory(kAiDir), [](const std::string& fn) {
		        return boost::ends_with(fn, kAiExtension) ||
