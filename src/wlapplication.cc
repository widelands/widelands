...
void WLApplication::handle_input(InputCallback const* cb) {
	...
	while (poll_event(ev)) {
		...
		case SDL_WINDOWEVENT:
			...
			SDL_GetWindowSize(g_gr->get_sdlwindow(), &window_w, &window_h);
			if (ev.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
				log("-- SDL_WINDOWEVENT_SIZE_CHANGED %dx%d (%dx%d%s)\n",
				    ev.window.data1, ev.window.data2, window_w, window_h, g_gr->fullscreen() ? " fs" : "");
			} else if (ev.window.event == SDL_WINDOWEVENT_RESIZED) {
				log("-- SDL_WINDOWEVENT_RESIZED      %dx%d (%dx%d%s)\n",
				    ev.window.data1, ev.window.data2, window_w, window_h, g_gr->fullscreen() ? " fs" : "");
