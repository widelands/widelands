...
bool is_autogenerated_and_expired(const std::string& filename, const double keep_time = kReplayKeepAroundTime) {
	tm tfile;
...
/**
 * Delete ai dna files generated during AI initialization
 */
void WLApplication::cleanup_ai_files() {
	for (const std::string& filename :
	     filter(g_fs->list_directory("ai"), // NOCOM repace with defineds
	        [](const std::string& fn) { return boost::ends_with(fn, "wai") ||
					boost::contains(fn, "ai_player");})) {
		if (is_autogenerated_and_expired(filename, kAIFilesKeepAroundTime)) {
			printf("Deleting generated ai file: %s\n", filename.c_str());
			g_fs->fs_unlink(filename); // uncomment
		}
	}
}
