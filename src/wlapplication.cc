...
std::shared_ptr<SDL_Cursor> try_load_mouse_cursor(const std::string& file_name, const Vector2i& hot_point)
{
	std::shared_ptr<SDL_Surface> cursor_surface(load_image_as_sdl_surface(file_name), SDL_FreeSurface);
	return cursor_surface
		? std::shared_ptr<SDL_Cursor>(
			SDL_CreateColorCursor(cursor_surface.get(), hot_point.x, hot_point.y),
			SDL_FreeCursor)
		: nullptr;
}

std::map<MouseCursorState, std::shared_ptr<SDL_Cursor>> try_load_standard_mouse_cursors()
{
	const Vector2i hot_point(3, 7);
	auto normal_cursor = try_load_mouse_cursor("images/ui_basic/cursor.png", hot_point);
	auto clicked_cursor = try_load_mouse_cursor("images/ui_basic/cursor_click.png", hot_point);

	std::map<MouseCursorState, std::shared_ptr<SDL_Cursor>> cursors;
	if (normal_cursor && clicked_cursor) {
		cursors = {
			{MouseCursorState::normal, normal_cursor},
			{MouseCursorState::clicked, clicked_cursor},
		};
	}
...
WLApplication::WLApplication(int const argc, char const* const* const argv)
   : commandline_(std::map<std::string, std::string>()),
	...
	sdl_cursors = try_load_standard_mouse_cursors();
	if (!sdl_cursors.empty()) {
		SDL_SetCursor(sdl_cursors[MouseCursorState::normal].get());
	} else {
...
void WLApplication::update_sdl_mouse_cursor() {
	auto cursor_state = is_mouse_pressed() ? MouseCursorState::clicked : MouseCursorState::normal;
	SDL_SetCursor(sdl_cursors[cursor_state].get());
}
