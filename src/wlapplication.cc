...
void WLApplication::setup_homedir() {
	...
	if (homedir_.empty()) {
		std::cout << "Unable to start Widelands, because the homedir is empty" << std::endl;
		delete g_fs;
		exit(1);
	} else {
#ifdef _WIN32
		...
		try {
			...
			log("Unable to start Widelands, because we were unable to add the home directory: %s\n", e.what());
			delete g_fs;
			exit(1);
		}
	}
