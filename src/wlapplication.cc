...
std::unique_ptr<FsMenu::MainMenu> WLApplication::check_crash_reports() {
	...
	for (const std::string& filename : g_fs->filter_directory(kCrashDir, [](const std::string& fn) { return ends_with(fn, kOldCrashExtension); })) {
	...
	FilenameSet crashes = g_fs->filter_directory(kCrashDir, [](const std::string& fn) { return ends_with(fn, kCrashExtension); });
	if (crashes.empty()) {
		return nullptr;
	}

	log_info("Found %" PRIuS " unsent crash reports.\nPlease consider submitting them to the Widelands Development Team under %s",
