...
void WLApplication::cleanup_replays() {
	for (const std::string& filename :
		...
		if (is_autogenerated_and_expired(filename)) {
			...
			assert(filename.length() > kSyncstreamExtension.length());
			std::string filename_wse = filename.substr(0, filename.length() - kSyncstreamExtension.length());
			filename_wse += kSyncstreamExcerptExtension;
			if (g_fs->file_exists(filename_wse)) {
				log("Delete syncstream excerpt %s\n", filename_wse.c_str());
				g_fs->fs_unlink(filename_wse);
			}
		}
	}
