...
Recti BaseListselect::get_highlight_rect(const std::string& text, int x, int y) {
        ...
        std::shared_ptr<const UI::RenderedText> rendered_start =
           UI::g_fh->render(as_richtext_paragraph(richtext_escape(text2.substr(0, start)), table_style().enabled()));

        std::shared_ptr<const UI::RenderedText> rendered_substring =
           UI::g_fh->render(as_richtext_paragraph(richtext_escape(text2.substr(start, filter.length())), table_style().enabled()));
