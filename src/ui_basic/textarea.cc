...
void Textarea::draw(RenderTarget & dst)
{
	if (!m_text.empty()) {
		Point anchor
			(static_cast<int>(m_align & UI::Align::Align_HCenter) ?
			 get_w() / 2 : static_cast<int>(m_align & UI::Align::Align_Right)  ? get_w() : 0,
			 static_cast<int>(m_align & UI::Align::Align_VCenter) ?
			 get_h() / 2 : static_cast<int>(m_align & UI::Align::Align_Bottom) ? get_h() : 0);
...
void Textarea::collapse()
{
	...
	if (static_cast<int>(m_align & UI::Align::Align_HCenter))
		x += w >> 1;
	else if (static_cast<int>(m_align & UI::Align::Align_Right))
		x += w;

	if (static_cast<int>(m_align & UI::Align::Align_VCenter))
		y += h >> 1;
	else if (static_cast<int>(m_align & UI::Align::Align_Bottom))
		y += h;
