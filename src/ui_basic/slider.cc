...
DiscreteSlider::DiscreteSlider(Panel* const parent,
   ...
   : Panel(parent, x, y, w, h, tooltip_text),
     slider(this,
            ...
            h - text_height(g_gr->styles().font_size(StyleManager::FontSize::kSlider), StyleManager::FontStyleInfo::Face::kCondensed) - 2,
            0,
            labels_in.size() - 1,
            value_,
            style,
...
void DiscreteSlider::draw(RenderTarget& dst) {
	...
	for (uint32_t i = 0; i < labels.size(); i++) {
		std::shared_ptr<const UI::RenderedText> rendered_text =
		   UI::g_fh1->render(as_condensed(labels[i], UI::Align::kCenter, g_gr->styles().font_size(StyleManager::FontSize::kSlider)));
		rendered_text->draw(
