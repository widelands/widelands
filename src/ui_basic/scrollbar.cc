...
void Scrollbar::draw(RenderTarget& dst) {
	...
	if (horizontal_) {
		...
		draw_area(
		   dst, Area::MinusPage, Recti(buttonsize_, 0, knobpos - buttonsize_ - knobsize / 2, get_h()));
		assert(knobpos + knobsize / 2 + buttonsize_ <= static_cast<uint32_t>(get_w()));
		draw_area(dst, Area::PlusPage, Recti(knobpos + knobsize / 2, 0,
		...
		assert(buttonsize_ + knobsize / 2 <= knobpos);
		draw_area(
		   dst, Area::MinusPage, Recti(0, buttonsize_, get_w(), knobpos - buttonsize_ - knobsize / 2));
