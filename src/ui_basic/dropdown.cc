...
void BaseDropdown::add(const std::string& name,
	...
	if (autoexpand_display_button_) {
		...
		const std::string fitme = label_.empty() ? name : (boost::format(_("%1%: %2%")) % label_ % name).str();
		const int new_width = text_width(richtext_escape(fitme), g_gr->styles().button_style(button_style_).enabled().font()) + 8;
