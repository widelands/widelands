...
void Window::on_resolution_changed_note(const GraphicResolutionChanged& note) {
    ...
    if (get_w() >= note.old_width && get_h() >= note.old_height) {
        // The window is sort-of fullscreen, e.g. help. So, we resize it.
		set_size(note.new_width, note.new_height);
		center_to_parent();
	} else {
        // Adjust x position
		if (abs(old_center_x - get_x() - get_w() / 2) < 2) {
            // The window was centered horizontally. Keep it that way.
			set_pos(Vector2i((note.new_width - get_w()) / 2, get_y()));
		} else if (get_x() > old_center_x) {
            // The window was in the right half of the screen. Shift to maintain distance to right edge of the screen.
			const int diff = note.old_width - get_x();
			set_pos(Vector2i(note.new_width - diff, get_y()));
		} else if (get_x() + get_w() > note.new_width) {
            // The window was in the left half of the screen, but doesn't fit any more. Shift to fit on screen.
			set_pos(Vector2i(note.new_width - get_w() + get_rborder(), get_y()));
		}
        // Adjust y position
		if (abs(old_center_y - get_y() - get_h() / 2) < 2) {
            // The window was centered vertically. Keep it that way.
			set_pos(Vector2i(get_x(), (note.new_height - get_h()) / 2));
		} else if (get_y() > old_center_y) {
            // The window was in the bottom half of the screen. Shift to maintain distance to bottom edge of the screen.
			const int diff = note.old_height - get_y();
			set_pos(Vector2i(get_x(), note.new_height - diff));
		} else if (get_y() + get_h() > note.new_height) {
            // The window was in the upper half of the screen, but doesn't fit any more. Shift to fit on screen.
