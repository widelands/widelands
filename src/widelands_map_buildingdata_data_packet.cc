...
#define CURRENT_MILITARYSITE_PACKET_VERSION     1
...
void Widelands_Map_Buildingdata_Data_Packet::read_militarysite(Building* building, FileRead* fr, Editor_Game_Base* egbase, Widelands_Map_Map_Object_Loader* ol) {
      ...
      if(version==CURRENT_MILITARYSITE_PACKET_VERSION) {
         ...
         // Request
         bool has_request = fr->Unsigned8();
         if(ms->m_soldier_request) 
            delete ms->m_soldier_request;
         
         if(has_request) {
		      ms->m_soldier_request = new Request(ms, 0, &MilitarySite::request_soldier_callback, ms, Request::WORKER);
            ms->m_soldier_request->Read(fr,egbase,ol);
         } else
            ms->m_soldier_request = 0;
...
void Widelands_Map_Buildingdata_Data_Packet::write_militarysite(Building* building, FileWrite* fw, Editor_Game_Base* egbase, Widelands_Map_Map_Object_Saver* os) {
   ...
   if(ms->m_soldier_request) {
      fw->Unsigned8(1);
      ms->m_soldier_request->Write(fw,egbase,os);
   } else
      fw->Unsigned8(0);
