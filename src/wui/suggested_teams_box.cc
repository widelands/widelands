	...
	m_label_height(label_height)
{
	...
	m_suggested_teams_box_label =
			new UI::Textarea(this, "", UI::Align::kCenterLeft);
	add(m_suggested_teams_box_label, UI::Align::kLeft);
}
		...
		for (const Widelands::Map::SuggestedTeamLineup& lineup : m_suggested_teams) {
			...
			for (const Widelands::Map::SuggestedTeam& team : lineup) {
				...
				if (!is_first) {
					...
					vs_label = new UI::Textarea(m_lineup_box, "x", UI::Align::kBottomCenter);
					m_lineup_box->add(vs_label, UI::Align::kLeft);
				...
				for (uint16_t player : team) {
					...
					player_icon->set_no_frame();
					m_lineup_box->add(player_icon, UI::Align::kLeft);
