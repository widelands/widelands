	...
	for (const Widelands::DescriptionIndex& index : items) {
		if (display_.ware_selected(index)) {
			...
			if (game) {
				...
				player_->get_economy(serial_)->set_target_quantity(
				   is_wares ? Widelands::wwWARE : Widelands::wwWORKER, index, new_amount, 0);
			}
		}
	}
}
	...
	for (const Widelands::DescriptionIndex& index : items) {
		if (display_.ware_selected(index) ||
			...
			if (game) {
				...
				player_->get_economy(serial_)->set_target_quantity(
				   is_wares ? Widelands::wwWARE : Widelands::wwWORKER, index, settings.wares.at(index),
				   0);
			}
		}
	}
