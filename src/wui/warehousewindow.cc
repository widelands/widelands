...
void WarehouseWaresDisplay::draw_ware(RenderTarget & dst, Widelands::Ware_Index ware)
{
	...
	case Warehouse::SP_DontStock: pic = g_gr->imgcache().get(pic_policy_dontstock); break;
	case Warehouse::SP_Remove: pic = g_gr->imgcache().get(pic_policy_remove); break;
	default:
...
#define ADD_POLICY_BUTTON(policy, policyname, tooltip)                                           \
		b = new UI::Button                                                             \
			(buttons, #policy, 0, 0, 34, 34,                                                  \
			 ...
			 g_gr->imgcache().get("pics/stock_policy_button_" #policy ".png"),      \
			 tooltip),                                                                        \
	...
	: Building_Window(parent, wh, registry)
{
	...
	if (Widelands::PortDock * pd = wh.get_portdock()) {
		get_tabs()->add
			("wares",
			 g_gr->imgcache().get(pic_tab_dock_wares),
			 create_portdock_wares_display(get_tabs(), Width, *pd, Widelands::wwWARE),
			 _("Wares in dock"));
		get_tabs()->add
			("workers",
			 g_gr->imgcache().get(pic_tab_dock_workers),
