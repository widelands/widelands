	...
	m_archivebtn =
		new UI::Button
			(this, "archive_or_restore_selected_messages",
			 ...
			 g_gr->imgcache().get("pics/message_archive.png", true),
	 ...
	 const Widelands::Message & message)
{
	er.set_picture
		(ColStatus,
		 g_gr->imgcache().get(status_picture_filename[message.status()], true));
...
void GameMessageMenu::toggle_mode()
{
	...
	case Inbox:
		...
		m_archivebtn->set_pic(g_gr->imgcache().get("pics/message_restore.png", true));
	...
	case Archive:
		...
		m_archivebtn->set_pic(g_gr->imgcache().get("pics/message_archive.png", true));
