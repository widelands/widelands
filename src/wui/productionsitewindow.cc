...
void ProductionSiteWindow::init(bool avoid_fastclick, bool workarea_preview_wanted) {
	...
	if (!inputqueues.empty()) {
		// Add the wares tab
		UI::Box* prod_box = new UI::Box(get_tabs(), UI::PanelStyle::kWui, "inputs_box", 0, 0, UI::Box::Vertical,
	...
	if (production_site->descr().nr_working_positions() == 0u) {
		...
		UI::Box* worker_box = new UI::Box(get_tabs(), UI::PanelStyle::kWui, "workers_box", 0, 0, UI::Box::Vertical);
		worker_table_ = new UI::Table<uintptr_t>(worker_box, "workers_table", 0, 0, 0, 100, UI::PanelStyle::kWui);
		worker_caps_ = new UI::Box(worker_box, UI::PanelStyle::kWui, "worker_caps_box", 0, 0, UI::Box::Horizontal);
