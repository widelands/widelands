...
m_infotext(this, _("Click soldier to send away"))
{
	...
	if (upcast(Widelands::MilitarySite, ms, &building)) {
		m_soldier_preference.add_button
			(buttons, Point(0, 0), g_gr->images().get("data/pics/prefer_rookies.png"), _("Prefer Rookies"));
		m_soldier_preference.add_button
			(buttons, Point(32, 0), g_gr->images().get("data/pics/prefer_heroes.png"), _("Prefer Heroes"));
