...
GameDiplomacyMenu::GameDiplomacyMenu(InteractiveGameBase& parent,
   ...
   : UI::UniqueWindow(
     ...
     trades_box_offers_(&trades_tabs_, UI::PanelStyle::kWui, "trades_offers", 0, 0, UI::Box::Vertical),
     trades_box_proposed_(&trades_tabs_, UI::PanelStyle::kWui, "trades_proposed", 0, 0, UI::Box::Vertical),
     trades_box_active_(&trades_tabs_, UI::PanelStyle::kWui, "trades_active", 0, 0, UI::Box::Vertical) {
	...
	trades_tabs_.add("active", _("Trade Offers"), &trades_box_offers_, _("Trade offers you have received from other players"));
	trades_tabs_.add("active", _("Proposed Trades"), &trades_box_proposed_, _("Trades you have proposed to other players"));
	trades_tabs_.add("active", _("Active Trades"), &trades_box_active_, _("Your active trade agreements"));
...
void GameDiplomacyMenu::update_trades_offers(bool always) {
	const std::vector<Widelands::TradeID> trades = iplayer_->game().find_trade_offers(iplayer_->player_number());
	...
	if (trades.empty()) {
		UI::MultilineTextarea* txt = new UI::MultilineTextarea(&trades_box_offers_, "text", 0, 0, 0, 0, UI::PanelStyle::kWui, std::string(),
				UI::mirror_alignment(UI::Align::kLeft, UI::g_fh->fontset()->is_rtl()), UI::MultilineTextarea::ScrollMode::kNoScrolling);
	...
	for (Widelands::TradeID trade_id : trades) {
		...
		UI::Box* box = new UI::Box(&trades_box_offers_, UI::PanelStyle::kWui, format("offer_%u", trade_id), 0, 0, UI::Box::Horizontal);
		...
		std::multimap<uint32_t, const Widelands::Market*> markets = iplayer_->player().get_markets(other_market->get_position());
		for (auto it = markets.rbegin(); it != markets.rend(); ++it) {
			const bool first = it == markets.rbegin();
			select_market->add(
				first ? format_l(_("%s (closest)"), it->second->get_market_name()) : it->second->get_market_name(),
				it->second->serial(), it->second->descr().icon(), first,
				format_l(ngettext("%1$s (distance: %2$u field)", "%1$s (distance: %2$u fields)", it->first), it->second->get_market_name(), it->first)
			);
		}

		UI::Button* yes = new UI::Button(buttons, "yes", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menu_okay.png"), _("Accept this trade offer"));
		UI::Button* no = new UI::Button(buttons, "no", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menu_abort.png"), _("Reject this trade offer"));
		...
		if (select_market->empty()) {
			...
			yes->sigclicked.connect([this, trade_id, select_market]() {
				iplayer_->game().send_player_trade_action(iplayer_->player_number(), trade_id, Widelands::TradeAction::kAccept, select_market->get_selected());
			});
		}
		no->sigclicked.connect([this, trade_id]() {
			iplayer_->game().send_player_trade_action(iplayer_->player_number(), trade_id, Widelands::TradeAction::kReject, 0);
		});
		...
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelHeading, format_l(_("Trade offer from %s"), other_market->owner().get_name()));

		infotext += "</p><p>";
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelParagraph, format_l(ngettext("%d batch", "%d batches", trade.trade.num_batches), trade.trade.num_batches));
		...
		box->add(new UI::MultilineTextarea(box, "description", 0, 0, 0, 0, UI::PanelStyle::kWui, infotext,
				UI::mirror_alignment(UI::Align::kLeft, UI::g_fh->fontset()->is_rtl()), UI::MultilineTextarea::ScrollMode::kNoScrolling), UI::Box::Resizing::kExpandBoth);
...
void GameDiplomacyMenu::update_trades_proposed(bool always) {
	const std::vector<Widelands::TradeID> trades = iplayer_->game().find_trade_proposals(iplayer_->player_number());
	...
	if (trades.empty()) {
		UI::MultilineTextarea* txt = new UI::MultilineTextarea(&trades_box_proposed_, "text", 0, 0, 0, 0, UI::PanelStyle::kWui, std::string(),
				UI::mirror_alignment(UI::Align::kLeft, UI::g_fh->fontset()->is_rtl()), UI::MultilineTextarea::ScrollMode::kNoScrolling);
		txt->set_style(UI::FontStyle::kWuiInfoPanelParagraph);
		txt->set_text(_("There are no pending trade proposals at the moment. You can propose a new trade from a market."));
	...
	for (Widelands::TradeID trade_id : trades) {
		...
		UI::Box* box = new UI::Box(&trades_box_proposed_, UI::PanelStyle::kWui, format("proposal_%u", trade_id), 0, 0, UI::Box::Horizontal);
		UI::Box* buttons = new UI::Box(box, UI::PanelStyle::kWui, "buttons", 0, 0, UI::Box::Vertical);

		UI::Button* go_to = new UI::Button(buttons, "go_to", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menus/goto.png"), _("Center view on this market"));
		UI::Button* cancel = new UI::Button(buttons, "cancel", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menu_abort.png"), _("Retract this trade offer"));
		...
		cancel->sigclicked.connect([this, trade_id]() {
			iplayer_->game().send_player_trade_action(iplayer_->player_number(), trade_id, Widelands::TradeAction::kRetract, 0);
		});
		...
		std::string infotext("<rt><p>");
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelHeading, format_l(
				/** TRANSLATORS: "From" is the market's name, and "to" is the receiving player's name */
				_("Trade offer from %1$s to %2$s"),
...
void GameDiplomacyMenu::update_trades_active(bool always) {
	const std::vector<Widelands::TradeID> trades = iplayer_->game().find_active_trades(iplayer_->player_number());
	...
	if (trades.empty()) {
		UI::MultilineTextarea* txt = new UI::MultilineTextarea(&trades_box_active_, "text", 0, 0, 0, 0, UI::PanelStyle::kWui, std::string(),
				UI::mirror_alignment(UI::Align::kLeft, UI::g_fh->fontset()->is_rtl()), UI::MultilineTextarea::ScrollMode::kNoScrolling);
		txt->set_style(UI::FontStyle::kWuiInfoPanelParagraph);
	...
	for (Widelands::TradeID trade_id : trades) {
		...
		UI::Box* box = new UI::Box(&trades_box_active_, UI::PanelStyle::kWui, format("active_%u", trade_id), 0, 0, UI::Box::Horizontal);
		UI::Box* buttons = new UI::Box(box, UI::PanelStyle::kWui, "buttons", 0, 0, UI::Box::Vertical);

		UI::Button* go_to = new UI::Button(buttons, "go_to", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menus/goto.png"), _("Center view on this market"));
		UI::Button* cancel = new UI::Button(buttons, "cancel", 0, 0, kRowSize, kRowSize, UI::ButtonStyle::kWuiSecondary, g_image_cache->get("images/wui/menu_abort.png"), _("Cancel this trade agreement"));
		...
		cancel->sigclicked.connect([this, trade_id]() {
			iplayer_->game().send_player_trade_action(iplayer_->player_number(), trade_id, Widelands::TradeAction::kCancel, 0);
		});
		...
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelHeading, format_l(
				...
				_("Trade agreement at %1$s with %2$s"),
				own_market->get_market_name(),
		...
		infotext += "</p><p>";
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelParagraph, format_l(ngettext("%d batch sent", "%d batches sent", batches_sent), batches_sent));

		infotext += "</p><p>";
		infotext += as_font_tag(UI::FontStyle::kWuiInfoPanelParagraph, format_l(ngettext("%d batch remaining", "%d batches remaining",
				trade.trade.num_batches - batches_sent), trade.trade.num_batches - batches_sent));
