...
void MessagePreview::draw(RenderTarget& r) {
	...
	r.tile(Recti(0, 0, get_w(), get_h()),
	       &load_safe_template_image("wui/windows/background.png"), Vector2i(0, 0));
...
void InfoPanel::rebuild_dropdown() {
	...
	toggle_mode_.add(_("Hide"), DisplayMode::kMinimized,
	                 &load_safe_template_image(on_top_ ? "wui/windows/minimize.png" :
	                                                                "wui/windows/maximize.png"),
	                 display_mode_ == DisplayMode::kMinimized);

	toggle_mode_.add(on_top_ ? _("Move panel to bottom") : _("Move panel to top"),
	                 DisplayMode::kCmdSwap,
	                 &load_safe_template_image(on_top_ ? "wui/windows/maximize.png" :
