...
bool BuildingStatisticsMenu::own_building_is_valid(const Widelands::Player& player,
	...
	// NOCOM(codereview): If a building needs seafaring or waterways and the map has no seafaring, we will not see it.
	// We should have a check function in the Player class that deals with this properly,
	// which we could then also use for the field actions.
	...
	// Skip waterways buildings if not needed
