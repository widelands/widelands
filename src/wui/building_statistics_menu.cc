...
BuildingStatisticsMenu::BuildingStatisticsMenu(InteractivePlayer& parent,
	...
	: UI::UniqueWindow(&parent,
		...
		building_name_(
			&navigation_panel_, get_inner_w() / 2, 0, 0, kButtonHeight, "", UI::Align::Align_Center),
	...
	for (int i = 0; i < kNoOfBuildingTabs; ++i) {
		tabs_[i]->add(rows[i], UI::Align::Align_Left);
	}
	...
	unproductive_box_.add(&unproductive_label_, UI::Align::Align_Left);
	unproductive_box_.add_space(2);
	unproductive_box_.add(&unproductive_percent_, UI::Align::Align_Left);
	unproductive_box_.add(&unproductive_label2_, UI::Align::Align_Left);
...
bool BuildingStatisticsMenu::add_button(
	...
	building_buttons_[id] = new UI::Button(button_box,
														...
														false,
														true);
	button_box->add(building_buttons_[id], UI::Align::Align_Left);

	owned_labels_[id] =
		new UI::Textarea(button_box, 0, 0, kBuildGridCellWidth, kLabelHeight, UI::Align::Align_Center);
	button_box->add(owned_labels_[id], UI::Align::Align_Left);

	productivity_labels_[id] =
		new UI::Textarea(button_box, 0, 0, kBuildGridCellWidth, kLabelHeight, UI::Align::Align_Center);
	button_box->add(productivity_labels_[id], UI::Align::Align_Left);

	row.add(button_box, UI::Align::Align_Left);
	...
	++*column;
	if (*column == kColumns) {
		tabs_[tab_index]->add(&row, UI::Align::Align_Left);
		*column = 0;
		return true;
	}
	return false;
