...
Serial MapObjectLoader::get_economy_savegame_compatibility(Serial ware_economy) const {
	if (ware_economy >= kEconomySavegameCompatibilityPrefix) {
		throw GameDataError("We are sorry, but this savegame is not compatible with the current Widelands "
				"version any more because it contains an economy with the unbelievably high serial %u", ware_economy);
	}
