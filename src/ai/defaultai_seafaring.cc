...
uint8_t DefaultAI::spot_scoring(Widelands::Coords candidate_spot) {
	...
	if (other_player_accessible(Player::AiPersistentState::kColonyScanMinArea, &tested_fields, &mineable_fields_count,
...
void DefaultAI::check_ship_in_expedition(ShipObserver& so, uint32_t const gametime) {
	...
	if (expedition_ship_ == kNoShip) {
		...
		const uint32_t expected_colony_scan =
		   Player::AiPersistentState::kColonyScanMinArea + (Player::AiPersistentState::kColonyScanStartArea - Player::AiPersistentState::kColonyScanMinArea) * remaining_time / 100;
