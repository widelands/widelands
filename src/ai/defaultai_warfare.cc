...
bool DefaultAI::check_trainingsites(uint32_t gametime) {
	...
	for (size_t i = 0; i < nr_warequeues; ++i) {
		...
		for (std::string pattern : armors_and_weapons) {
			...
			if (tribe_->get_ware_descr(warequeues1[i]->get_type())->name().find(pattern) !=
			    std::string::npos) {
				if (warequeues1[i]->get_max_fill() > 1) {
					game().send_player_set_input_max_fill(*ts, warequeues1[i]->get_type(), wwWARE, 1);
					continue;
				}
			}
		}
	...
	if (tso.site->soldier_capacity() == 0) {
		...
		for (size_t i = 0; i < nr_warequeues; ++i) {
			if (tso.bo->substitute_inputs.count(warequeues2[i]->get_type()) > 0) {
