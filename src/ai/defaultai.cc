		...
		if (upcast(Building const, building, &base_immovable)) {
			if (upcast(ConstructionSite const, constructionsite, building)) {
				...
				if (upcast(MilitarySite_Descr const, target_ms_d, &target_descr)) {
					...
					if (v > 0) {
						if (kMilitScoreDebug)
							printf(" FIELD SCORE:  testing near military construction site at %3dx%3d, "
			...
			if (upcast(MilitarySite const, militarysite, building)) {
				...
				if (v > 0 and dist > 0) {
					if (kMilitScoreDebug)
						printf(" FIELD SCORE:  testing near military building at %3dx%3d, capacity: %d, "
...
bool DefaultAI::construct_building(int32_t gametime) {  // (int32_t gametime)
			...
			if (bo.type == BuildingObserver::PRODUCTIONSITE) {
				if (bo.need_trees_) {  // LUMBERJACS
					...
					if (kHintDebug or(kWoodDebug and bo.plants_trees_))
						printf(" TDEBUG: %1d: suggesting new %16s at %3dx%3d, current count: %2d, goal: "
					...
					if ((bo.is_basic_ or bo.prod_build_material_)and bo.total_count() == 0)
						...
						if (kSpaceDebug and bo.space_consumer_)
							printf(" TDEBUG: %1d: considering %-15s, stock: %3d\n",
							       player_number(),
							       bo.name,
							       bo.stocklevel_);
						...
						if (bo.stocklevel_ < 50) {
							...
							if (kProductionDebug or(kSpaceDebug and bo.space_consumer_))
								printf(" TDEBUG: %1d: proposing %-15s , on stock: %3d(<50), stat: %3d, "
						...
						// if output is needed and there are no idle buildings
							...
							if (bo.cnt_built_ > 0 and bo.current_stats_ > 80) {
								...
								if (kProductionDebug)
									printf(" TDEBUG: %2d/%-15s with inputs_: stats: %3d>90, setting "
								...
								if (kProductionDebug)
									printf(" TDEBUG: %2d/%-15s with inputs_: stats: %3d>60, setting "
									       "priority: %2d, on %3d %3d\n",
									       bo.id,
									       bo.name,
									       bo.current_stats_,
				...
				if (kMilitaryDebug)
					printf(" NEW MILITARY: %3dx%3d: unowned: %3d(%3d), mines_: %3d, lonel.: %4d (%4d), "
	...
	if (mine) {
		...
		if (kMinesUpdateDebug)
			printf("  new next_mine_construction_due_=%10d\n", next_mine_construction_due_);
...
bool DefaultAI::check_productionsites(int32_t gametime) {
	...
	if (site.bo->inputs_.empty()  // does not consume anything
		...
		if (site.bo->cnt_built_ > site.bo->cnt_target_) {
			...
			if (kSpaceDebug)
				printf(" TDEBUG: considering dismantle of space consumer: %15s, count %2d/T:%2d, stock "
				       "level:%3d(>100)\n",
				       site.bo->name,
				       site.bo->cnt_built_,
				       site.bo->cnt_target_,
	...
	if (site.bo->inputs_.size() > 0 and site.bo->cnt_built_ >= 3 and site.bo->current_stats_ < 30) {
		if (kIdleDismantle)
			printf(" kIdleDismantle: dismantling due to too many buildings: %15s at %3d x %3d, total "
			       "counts: %2d, stat: %2d\n",
			       site.bo->name,
			       site.site->get_position().x,
			       site.site->get_position().y,
	...
	if (site.bo->production_hint_ >= 0) {
		// if (kStandbyDebug) printf ("  TDEBUG: check_productionsites(): testing building
		// %s\n",site.bo->name);
		if (site.bo->stocklevel_time < game().get_gametime() - 5 * 1000) {
	...
	container_iterate_const(std::set<Building_Index>, enhancements, x) {
		...
		if (player->is_building_type_allowed(*x.current)) {
			...
			if (en_bo.current_stats_ > 65) {
				...
				if (kUpgradeDebug)
					printf(" UPGRADE:  proposing upgrade (non-first building) %12s at %3d x %3d: prio: "
...
bool DefaultAI::check_mines_(int32_t const gametime) {
	...
	container_iterate_const(std::set<Building_Index>, enhancements, x) {
		...
		if (player->is_building_type_allowed(*x.current)) {
			...
			if (kMinesUpdateDebug)
				printf(" MINES_UPDATE:   considering upgrade to %15s, count B:%1d(stat:%3d)  U:%1d  "
				       "C:%1d\n",
				       en_bo.name,
				       en_bo.cnt_built_,
				       en_bo.current_stats_,
	...
	if (map.find_fields(Area<FCoords>(f, vision), nullptr, find_enemy) == 0) {
		...
		if (ms->economy().warehouses().size()) {
			...
			// building to free some building space.
				...
				if (kMilDismDebug)
					printf(" DISMANTLE CHECK: testing finished building %3dx%3d, capacity: %2d(>9), "
