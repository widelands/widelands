...
void show_usage() {
	...
	std::cout << std::endl
	          ...
	          << "       " << _("widelands <save.wgf>/<replay.wrpl>") << std::endl
	          << std::endl;

	std::cout << _("General options:") << std::endl << std::endl;
	print_help_option(
	   _("--<config-entry-name>=value"), _("Overwrites any config file setting. Note: The new value "
	                                       "will be written to the config file."));
#ifndef _WIN32
	print_help_option(
	   _("--homedir=DIRNAME"), _("Use specified directory for Widelands config files, savegames and "
	                             "replays. Default is ~/.widelands"));
#else
	print_help_option(
	   _("--homedir=DIRNAME"), _("Use specified directory for Widelands config files, savegames and "
	                             "replays. Default is %USERPROFILE%\\.widelands"));
#endif

	print_help_option(
	   _("--localedir=DIRNAME"), _("Use specified directory for the Widelands locale files."));
	print_help_option(
	...
	print_help_option(
	   ...
	   /** TRANSLATORS: A syncstream is a synchronization stream. Syncstreams are used in multiplayer
	      games to make sure that there is no mismatch between the players. */
	   _("--write_syncstreams=[true|false]"),
	   _("Create syncstream dump files to help debug network games."));
	print_help_option(_("--autosave=[...]"), _("Automatically save each n minutes."));
	print_help_option(
	   _("--rolling_autosave=[...]"), _("Use this many files for rolling autosaves."));
	print_help_option(
	   _("--metaserver=[...]"), _("Connect to a different metaserver for internet gaming."));
	print_help_option(
	   _("--metaserverport=[...]"), _("Port number of the metaserver for internet gaming."));
	print_help_option(
	   _("--addon_server=[...]"),
	   _("Connect to a different github repository and branch from the add-ons manager."));
	print_help_option(_("--nosound"), _("Starts the game with sound disabled."));
	print_help_option(
	   _("--fail-on-lua-error"), _("Force Widelands to crash when a Lua error occurs."));
	...
	print_help_option(_("--editor=FILENAME"),
	                  _("Directly starts the Widelands editor and loads the map FILENAME."));
	print_help_option(
	   _("--ai_training"),
	...
	print_help_option(
	   _("--auto_speed"),
	   _("In multiplayer games only, this will keep adjusting the game speed automatically, "
	     "depending on FPS. Useful in conjunction with --ai_training."));
	print_help_option(_("--new_game_from_template=FILENAME"),
	                  _("Directly create a new singleplayer game configured in the given file. An "
	                    "example can be found in `data/templates/new_game_template`."));
	print_help_option(
	   _("--scenario=FILENAME"), _("Directly starts the map FILENAME as scenario map."));
	...
	print_help_option(
	   _("--script=FILENAME"), _("Run the given Lua script after initialization. Only valid with "
	                             "--scenario, --loadgame, or --editor."));
	print_help_option(
	   /** TRANSLATORS: You may translate true/false, also as on/off or yes/no, but it HAS TO BE
	      CONSISTENT with the translation in the widelands textdomain */
	   _("--auto_roadbuild_mode=[true|false]"),
	   _("Whether to enter roadbuilding mode automatically after placing a flag that is not "
	     "connected to a road."));
	...
	std::cout << std::endl
	          << _("Options for the internal window manager:") << std::endl
	          << std::endl;
	print_help_option(
	   /** TRANSLATORS: You may translate true/false, also as on/off or yes/no, but it HAS TO BE
	      CONSISTENT with the translation in the widelands textdomain */
	...
	print_help_option(_("--panel_snap_distance=[0 ...]"),
	                  _("Move a window to the edge of the panel when the edge of the window comes "
	                    "within this distance from the edge of the panel."));
	print_help_option(
	   /** TRANSLATORS: You may translate true/false, also as on/off or yes/no, but it HAS TO BE
	      CONSISTENT with the translation in the widelands textdomain */
	   _("--snap_windows_only_when_overlapping=[true|false]"),
	...
	std::cout << std::endl
	          << _("Bug reports? Suggestions? Check out the project website:") << std::endl
	          << "        https://www.widelands.org/" << std::endl
