	...
	if (production_site->descr().nr_working_positions() == 0u) {
		...
		if (ibase()->omnipotent()) {
			...
			worker_xp_decrease_ = new UI::Button(
			   worker_caps_, "xp_decrease", 0, 0, default_button_size(), default_button_size(),
			...
			worker_xp_increase_ = new UI::Button(
			   worker_caps_, "xp_increase", 0, 0, default_button_size(), default_button_size(),
		...
		if (ibase()->can_act(production_site->owner().player_number())) {
			UI::Button* evict_button =
			   new UI::Button(worker_caps_, "evict", 0, 0, default_button_size(),
