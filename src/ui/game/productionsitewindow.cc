	...
	if (production_site->descr().nr_working_positions() == 0u) {
		...
		if (ibase()->omnipotent()) {
			...
			worker_type_ = new UI::Dropdown<Widelands::DescriptionIndex>(
			   worker_caps_, "worker_type", 0, 0, 100, 8, default_button_size(), _("Worker"),
			   UI::DropdownType::kTextual, UI::PanelStyle::kWui, UI::ButtonStyle::kWuiMenu);
			worker_xp_decrease_ = new UI::Button(
			   worker_caps_, "xp_decrease", 0, 0, default_button_size(), default_button_size(), UI::ButtonStyle::kWuiMenu,
			   g_image_cache->get("images/ui_basic/scrollbar_down.png"),
			   _("Decrease experience by 1"));
			worker_xp_increase_ = new UI::Button(
			   worker_caps_, "xp_increase", 0, 0, default_button_size(), default_button_size(), UI::ButtonStyle::kWuiMenu,
			...
			worker_caps_->add_space(default_button_size());
		...
		if (ibase()->can_act(production_site->owner().player_number())) {
			UI::Button* evict_button = new UI::Button(
			   worker_caps_, "evict", 0, 0, default_button_size(), default_button_size(), UI::ButtonStyle::kWuiMenu,
