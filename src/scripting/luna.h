 ...
 *
 * It is written by Lenny Palozzi
 *
 * http://www.lua.org/notes/ltn005.html
 *
 * It was slightly adapted here and there to fit the needs
 * of Widelands
 */
...
public:
  ...
  static void Register(lua_State *L, const char* in_lib = 0) {
    ...
    // scripts can add functions written in Lua.
	 if(!in_lib) {
		 ...
		 lua_getfield(L, -1, in_lib);  /* get _LOADED[libname] */
		 lua_pushstring(L, T::className);
		 lua_pushvalue(L, methods);
		 lua_settable(L, -3);
	 }
  ...
  static T *check(lua_State *L, int narg) {
    ...
    if(!ud)
		 throw LuaValueError(T::className);
