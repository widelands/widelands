...
#ifndef S__LUA_IMPL_H
#define S__LUA_IMPL_H
 ...
 */
template <class T>
T** m_get_user_class_from_table(lua_State* L) {
 // GET table[ 0 ]
 lua_pushnumber( L, 0 );
 lua_rawget( L, 1 );
...
template <class T>
const PropertyType<T>* m_lookup_property_in_metatable(lua_State* L) {
 ...
 */
template <class T>
int m_property_getter(lua_State *L) {
	...
	T** obj = m_get_user_class_from_table<T>(L);
...
template <class T>
int m_property_setter(lua_State *L) {
	const PropertyType<T>* list = m_lookup_property_in_metatable<T>(L);
 ...
 */
template <class T>
int m_method_dispatch( lua_State *L ) {
	// Check for invalid: obj.method()
	int n = lua_gettop(L);
	if(!n)
		return report_error(
				L, "Method needs at least the object as argument!");
 ...
 */
template <class T>
int m_garbage_collect( lua_State *L ) {
 ...
 */
template <class T>
int m_constructor(lua_State *L) {
	return to_lua<T>(L, new T(L));
}

template <class T>
void m_add_constructor_to_lua(lua_State* L) {
...
template <class T>
int m_create_metatable_for_class(lua_State* L) {
...
template <class T>
void m_register_properties_in_metatable(lua_State* L, int metatable) {
	for ( int i = 0; T::Properties[i].name; i++ ) {
		// metatable[prop_name] = Pointer to getter setter
		lua_pushstring( L, T::Properties[i].name );
		lua_pushlightuserdata( L,
				const_cast<void*>(
					reinterpret_cast<const void*>(&T::Properties[i])));
		lua_settable( L, metatable );
	}
}

template <class T, class PT>
void m_register_methods_in_metatable
	(lua_State* L, int metatable)
{
	...
	for ( int i = 0; PT::Methods[i].name; i++ ) {
		...
		lua_pushlightuserdata( L,
				const_cast<void*>(
					reinterpret_cast<const void*>(&PT::Methods[i].method)));
...
#endif /* end of include guard: S__LUA_IMPL_H */
