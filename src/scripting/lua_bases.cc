...
int LuaPlayerBase::place_bob(lua_State * L) {
	...
	const std::string& shipname = egbase.tribes().get_ship_descr(ship_index)->name();
	Bob& bob = egbase.create_bob(c->coords(), shipname, MapObjectDescr::OwnerType::kTribe, &player);
