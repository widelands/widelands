...
//
// The :mod:`wl.bases` module contains Base functions on which the classes of
// :mod:`wl.game` and :mod:`wl.editor` are based. You will not need to create
// any of the functions in this module directly, but you might use their
// functionality via the child classes.
...
void L_EditorGameBase::__persist(lua_State * const L) {
}
void L_EditorGameBase::__unpersist(lua_State * const L) {
}
...
int L_EditorGameBase::get_map(lua_State * const L) {
...
// .. attribute:: players
//
//    (RO) An :class:`array` with the defined players. The players are either
//    of type :class:`wl.game.Player` or :class:`wl.editor.Player`.
//
...
int L_EditorGameBase::get_players(lua_State * const L) {
...
void L_PlayerBase::__persist(lua_State * const L) {
	PERS_UINT32("player", m_pl);
}
void L_PlayerBase::__unpersist(lua_State * const L) {
...
int L_PlayerBase::get_number(lua_State * const L) {
...
int L_PlayerBase::get_tribe(lua_State * const L) {
...
/*
 ...
 ==========================================================
 */
int L_PlayerBase::__eq(lua_State * const L) {
	Editor_Game_Base & egbase = get_egbase(L);
...
int L_PlayerBase::__tostring(lua_State * const L) {
	char rv[40];
	snprintf
...
//
//    Builds a flag on a given field if it is legal to do so. If not, reports
//    an error.
//
//    :arg field: where the flag should be created
//    :type field: :class:`wl.map.Field`
//    :arg force: If this is :const:`true` then the flag is created with pure
//    force:
//
//          * if there is an immovable on this field, it will be removed
//          * if there are flags too close by to this field, they will be
//            ripped
//          * if the player does not own the territory, it is conquered for
//            him.
//    :type force: :class:`boolean`
//    :returns: :class:`wl.map.Flag` object created or :const:`nil`.
int L_PlayerBase::place_flag(lua_State * const L) {
...
//
//    Start a road at the given field, then walk the directions given. Places a
//    flag at the last field.
//
//    If the last argument to this function is :const:`true` the road will be
//    created by force: all immovables in the way are removed and land is
//    conquered.
//
...
int L_PlayerBase::place_road(lua_State * const L) {
	Editor_Game_Base & egbase = get_egbase(L);
	Map & map = egbase.map();
...
//    out completely empty. If :const:`cs` is set, the building is not created
//    directly, instead a constructionsite for this building is placed.
//
...
int L_PlayerBase::place_building(lua_State * const L) {
...
//    :returns: :const:`nil`
int L_PlayerBase::conquer(lua_State * const L) {
	uint32_t radius = 1;
	if (lua_gettop(L) > 2)
