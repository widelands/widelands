...
int LuaPlayer::seen_field(lua_State* L) {
	...
	lua_pushboolean(L, get(L, egbase).get_vision(i) != Widelands::SeeUnseeNode::kUnexplore);
	return 1;
}
...
int LuaPlayer::reveal_fields(lua_State* L) {
	...
	while (lua_next(L, 2) != 0) {
		p.hide_or_reveal_field((*get_user_class<LuaField>(L, -1))->coords(), SeeUnseeNode::kReveal);
		lua_pop(L, 1);
	}
