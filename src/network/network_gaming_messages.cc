...
const std::string NetworkGamingMessages::get_message(const std::string& code,
	...
	for (uint8_t i = 0; i < ngmessages[code].n_arg; ++i) {
		if (args[i].empty()) {
			log_warn("NetworkGamingMessages::get_message: %s: missing argument %d", code.c_str(), i+1);
		}
	}
	for (uint8_t i = ngmessages[code].n_arg; i < 3; ++i) {
		if (!args[i].empty()) {
			log_warn("NetworkGamingMessages::get_message: %s: unexpected argument %d: %s", code.c_str(),
			   i+1, args[i].c_str());
		}
	}
...
void NetworkGamingMessages::fill_map() {
	...
	ngmessages["PLAYERCMD_WO_GAME"] = { _("Received a PLAYERCOMMAND even though no game is running."), 0};
	...
	ngmessages["PROTOCOL_EXCEPTION"] = { _("Received command number %s,"
	                                     " which is not allowed in this state."), 1};
