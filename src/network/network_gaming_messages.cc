...
static const std::map<std::string, std::string> ngmessages = {
   ...
   {"GAME_STARTED_AT_CONNECT", gettext_noop("The game has started just after you tried to connect.")},
   ...
   {"SYNCREQUEST_WO_GAME", gettext_noop("Server sent a SYNCREQUEST even though no game is running.")},
   ...
   {"PLAYER_UPDATE_FOR_N_E_P", gettext_noop("Server sent a player update for a player that does not exist.")},
   {"USER_UPDATE_FOR_N_E_U", gettext_noop("Server sent a user update for a user that does not exist.")},
   {"DIFFERENT_PROTOCOL_VERS", gettext_noop("Server uses a different protocol version")},
   {"PROTOCOL_EXCEPTION", gettext_noop("Received command number %s, which is not allowed in this state.")},
   {"BACKWARDS_RUNNING_TIME", gettext_noop("Client reports time to host that is running backwards.")},
   {"SIMULATING_BEYOND_TIME", gettext_noop("Client simulates beyond the game time allowed by the host.")},
   ...
   {"TIME_SENT_NOT_READY", gettext_noop("Client sent TIME command even though game is not running.")},
   ...
   {"REQUEST_OF_N_E_FILE", gettext_noop("Client requests file although none is available to send.")},
   {"SENT_PLAYER_TO_LOBBY", gettext_noop("Host sent player %s to the lobby!")},
   {"DIFFERENT_WL_VERSION", gettext_noop("WARNING: %1$s uses version: %2$s, while Host uses version: %3$s")},
