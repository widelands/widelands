...
const std::string NetworkGamingMessages::get_message(const std::string& code,
	...
	if (ngmessages.find(code) == ngmessages.end()) {
		return (bformat("%s, %s, %s, %s", code, get_message(arg1), get_message(arg2),
		         get_message(arg3)));
	}
	...
	while (strings.size() > 1 && last > strings.size()) {
		...
		try {
			...
			std::string temp =
			   bformat(strings.at(strings.size() - 2), strings.at(strings.size() - 1));
			...
			try {
				...
				std::string temp = bformat(strings.at(strings.size() - 3),
	...
	// No, it did not
	return (bformat("%s, %s, %s, %s", get_message(code), get_message(arg1),
