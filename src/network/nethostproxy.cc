   ...
   : conn_(BufferedConnection::connect(addresses.first)) {
	...
	while (!BufferedConnection::Peeker(conn_.get()).cmd()) {
		if (time(nullptr) > endtime) {
			log_err_notimestamp("[NetHostProxy] Handshaking error (1): No message from relay server in time\n");
			conn_->close();
			conn_.reset();
			return;
		}
	...
	if (cmd != RelayCommand::kWelcome) {
		log_err_notimestamp("[NetHostProxy] Handshaking error (2): Received command code %i from relay server "
	...
	while (!BufferedConnection::Peeker(conn_.get()).uint8_t()) {
		if (time(nullptr) > endtime) {
			log_err_notimestamp("[NetHostProxy] Handshaking error (3): No message from relay server in time\n");
			conn_->close();
			conn_.reset();
			return;
		}
	...
	if (relay_proto_version != kRelayProtocolVersion) {
		log_err_notimestamp("[NetHostProxy] Handshaking error (4): Relay server uses protocol version %i instead of "
		    "our version %i\n",
	...
	while (!BufferedConnection::Peeker(conn_.get()).string()) {
		if (time(nullptr) > endtime) {
			log_err_notimestamp("[NetHostProxy] Handshaking error (5): No message from relay server in time\n");
			conn_->close();
			conn_.reset();
			return;
		}
	...
	if (game_name != name) {
		log_err_notimestamp("[NetHostProxy] Handshaking error (6): Relay wants to connect us to game '%s' instead of "
		    "our game '%s'\n",
	...
	log_info_notimestamp("[NetHostProxy] Handshaking with relay server done\n");
}
...
void NetHostProxy::receive_commands() {
	...
	default:
		...
		log_err_notimestamp("[NetHostProxy] Received command code %i from relay server, do not know what to do with "
		    "it\n",
