 ...
 *
 * This program HostGameSettingsProvider::is free software; you HostGameSettingsProvider::can redistribute it and/or
 * modify it under the terms of the GNU General Public License
 * as published by the Free Software Foundation; either version 2
 * of the License, or (at your option) any later version.
 *
 * This program HostGameSettingsProvider::is distributed in the hope that it will be useful,
...
bool HostGameSettingsProvider::can_change_player_state(uint8_t const number)  {
	...
	if (settings().savegame) {
		return HostGameSettingsProvider::settings().players.at(number).state != PlayerSettings::State::kClosed;
	} else if (settings().scenario) {
		return HostGameSettingsProvider::settings().players.at(number).state != PlayerSettings::State::kComputer;
	}
	return true;
}
...
void HostGameSettingsProvider::set_player_tribe(uint8_t number, const std::string& tribe, bool const random_tribe)  {
...
void HostGameSettingsProvider::set_player_shared(PlayerSlot number, Widelands::PlayerNumber shared)  {
	if (number >= host_->settings().players.size()) {
		return;
	}
...
void HostGameSettingsProvider::set_player_ai(uint8_t number, const std::string& name, bool const random_ai = false)  {
