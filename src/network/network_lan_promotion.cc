...
LanBase::LanBase(uint16_t port) : io_service(), socket_v4(io_service), socket_v6(io_service) {
	...
	if (!is_open()) {
		...
		log_info_notimestamp("[LAN] Trying to open both sockets.\n");
		start_socket(&socket_v4, boost::asio::ip::udp::v4(), port);
		start_socket(&socket_v6, boost::asio::ip::udp::v6(), port);
	}
	...
	if (!is_open()) {
		...
		log_err_notimestamp("[LAN] Error: No sockets could be opened.\n");
		report_network_error();
	}

	for (const std::string& ip : broadcast_addresses_v4) {
		log_info_notimestamp("[LAN] Will broadcast to %s.\n", ip.c_str());
	}
	if (socket_v6.is_open()) {
		log_info_notimestamp("[LAN] Will broadcast for IPv6.\n");
	}
}
...
bool LanBase::is_available() {
	const auto do_is_available = [this](boost::asio::ip::udp::socket& socket) -> bool {
		...
		if (ec) {
			log_err_notimestamp("[LAN] Error when checking whether data is available on IPv%d socket, closing it: "
			    "%s.\n",
	...
	const auto do_receive = [this, &buf, &len, &recv_len,
		...
		if (socket.is_open()) {
			try {
				...
				log_err_notimestamp("[LAN] Error when receiving data on IPv%d socket, closing it: %s.\n",
...
bool LanBase::send(void const* const buf, size_t const len, const NetAddress& addr) {
	...
	if (!socket->is_open()) {
		...
		log_err_notimestamp("[LAN] Error: trying to send to an IPv%d address but socket is not open.\n",
		    get_ip_version(addr.ip));
		return false;
	}
	socket->send_to(boost::asio::buffer(buf, len), destination, 0, ec);
	if (ec) {
		log_err_notimestamp("[LAN] Error when trying to send something over IPv%d, closing socket: %s.\n",
...
#endif  // __APPLE__
				log_err_notimestamp("[LAN] Error when broadcasting on IPv%d socket to %s, closing it: %s.\n",
				    get_ip_version(destination.address()), address.c_str(), ec.message().c_str());
				close_socket(&socket);
#ifdef __APPLE__
			} else {
				log_err_notimestamp("[LAN] Error when broadcasting on IPv6 socket to %s: %s.\n", address.c_str(),
				    ec.message().c_str());
			}
#endif  // __APPLE__
		}
	...
	for (auto it = interface_indices_v6.begin(); it != interface_indices_v6.end();) {
		...
		if (!success) {
			...
			if (interface_indices_v6.empty()) {
				log_warn_notimestamp("[LAN] Warning: No more multicast capable IPv6 interfaces. "
...
void LanBase::start_socket(boost::asio::ip::udp::socket* socket,
	...
	if (ec) {
		log_err_notimestamp("[LAN] Failed to start an IPv%d socket: %s.\n", get_ip_version(version),
		    ec.message().c_str());
		return;
	}
	...
	if (ec) {
		log_err_notimestamp("[LAN] Error setting options for IPv%d socket, closing socket: %s.\n",
	...
	if (ec) {
		log_err_notimestamp("[LAN] Error binding IPv%d socket to UDP port %d, closing socket: %s.\n",
		    get_ip_version(version), port, ec.message().c_str());
		close_socket(socket);
		return;
	}

	log_info_notimestamp("[LAN] Started an IPv%d socket on UDP port %d.\n", get_ip_version(version), port);
}
...
void LanBase::close_socket(boost::asio::ip::udp::socket* socket) {
	...
	if (socket->is_open()) {
		...
		if (!ec) {
			log_info_notimestamp("[LAN] Closing an IPv%d socket.\n", get_ip_version(endpoint.protocol()));
		}
...
void LanGamePromoter::run() {
	...
	while (is_available()) {
		...
		log_info_notimestamp("Received %s packet from %s\n", magic, addr.ip.to_string().c_str());
