...
public:
	 ...
	 * \param priority The priority to send the packet with. Packets with higher priority (e.g., pings)
	 * might overtake packets with lower priority (e.g., file transfers).
	 */
	virtual void send(ConnectionId id, const SendPacket& packet, NetPriority priority = NetPriority::kNormal) = 0;
	 ...
	 * might overtake packets with lower priority (e.g., file transfers).
	 */
	virtual void send(const std::vector<ConnectionId>& ids, const SendPacket& packet, NetPriority priority = NetPriority::kNormal) = 0;
};
