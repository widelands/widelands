...
local int32_t unzlocal_getByte OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                   voidpf filestream,
                                   int32_t* pi));

local int32_t unzlocal_getByte(zlib_filefunc_def const* const pzlib_filefunc_def,
                               voidpf filestream,
                               int32_t* const pi) {
	unsigned char c;
...
*/
local int32_t unzlocal_getShort OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                    voidpf filestream,
                                    uLong* pX));

local int32_t unzlocal_getShort(zlib_filefunc_def const* const pzlib_filefunc_def,
                                voidpf filestream,
                                uLong* const pX) {
	uLong x;
	int32_t i = 0;
...
local int32_t unzlocal_getLong OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                   voidpf filestream,
                                   uLong* pX));

local int32_t unzlocal_getLong(zlib_filefunc_def const* const pzlib_filefunc_def,
                               voidpf filestream,
                               uLong* const pX) {
	uLong x;
	int32_t i = 0;
...
/*
  Locate the Central directory of a zipfile (at the end, just before
    the global comment)
*/
local uLong unzlocal_SearchCentralDir OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                          voidpf filestream));

local uLong unzlocal_SearchCentralDir(zlib_filefunc_def const* pzlib_filefunc_def,
                                      voidpf filestream) {
	unsigned char* buf;
	uLong uSizeFile;
	uLong uBackRead;
	...
	while (uBackRead < uMaxBack) {
		...
		if (ZSEEK(*pzlib_filefunc_def, filestream, uReadPos, ZLIB_FILEFUNC_SEEK_SET))
			break;
...
*/
extern unzFile ZEXPORT unzOpen2(char const* path, zlib_filefunc_def* const pzlib_filefunc_def) {
	unz_s us;
	...
	us.filestream = (*(us.z_filefunc.zopen_file))(
	   us.z_filefunc.opaque, path, ZLIB_FILEFUNC_MODE_READ | ZLIB_FILEFUNC_MODE_EXISTING);
	...
	if (ZSEEK(us.z_filefunc, us.filestream, central_pos, ZLIB_FILEFUNC_SEEK_SET))
		err = UNZ_ERRNO;
	...
	if  //  total number of entries in the central dir
	   (unzlocal_getShort(&us.z_filefunc, us.filestream, &number_entry_CD) != UNZ_OK)
		err = UNZ_ERRNO;
	...
	if (central_pos < us.offset_central_dir + us.size_central_dir and err == UNZ_OK)
		err = UNZ_BADZIPFILE;
...
local int32_t unzlocal_GetCurrentFileInfoInternal(unzFile file,
	...
	if (err == UNZ_OK and (szFileName != nullptr)) {
		uLong uSizeRead;
		...
		if (file_info.size_filename > 0 && fileNameBufferSize > 0 and
		    ZREAD(s->z_filefunc, s->filestream, szFileName, uSizeRead) != uSizeRead)
			err = UNZ_ERRNO;
		lSeek -= uSizeRead;
	}
	...
	if (err == UNZ_OK && (extraField != nullptr)) {
		uLong uSizeRead;
		uSizeRead = file_info.size_file_extra < extraFieldBufferSize ? file_info.size_file_extra :
		...
		if (file_info.size_file_extra > 0 && extraFieldBufferSize > 0 and
		    ZREAD(s->z_filefunc, s->filestream, extraField, uSizeRead) != uSizeRead)
			err = UNZ_ERRNO;
		lSeek += file_info.size_file_extra - uSizeRead;
	} else
		lSeek += file_info.size_file_extra;

	if (err == UNZ_OK and (szComment != nullptr)) {
		uLong uSizeRead;
...
extern int32_t ZEXPORT unzGetCurrentFileInfo(unzFile file,
                                             ...
                                             char* const szComment,
                                             uLong commentBufferSize) {
	return unzlocal_GetCurrentFileInfoInternal(file, pfile_info, nullptr, szFileName,
	                                           fileNameBufferSize, extraField, extraFieldBufferSize,
...
extern int32_t ZEXPORT unzGoToFirstFile(unzFile file) {
	...
	err = unzlocal_GetCurrentFileInfoInternal(
	   file, &s->cur_file_info, &s->cur_file_info_internal, nullptr, 0, nullptr, 0, nullptr, 0);
	s->current_file_ok = static_cast<uLong>(err == UNZ_OK);
	return err;
}
...
//  extra field data).
local int32_t unzlocal_CheckCurrentFileCoherencyHeader(unz_s* const s,
	...
	*psize_local_extrafield = 0;
	...
	if (unzlocal_getLong(&s->z_filefunc, s->filestream, &uData) != UNZ_OK)
		err = UNZ_ERRNO;
	else if (err == UNZ_OK and uData != s->cur_file_info.compressed_size and (uFlags & 8) == 0)
		err = UNZ_BADZIPFILE;

	/* size uncompr */
	if (unzlocal_getLong(&s->z_filefunc, s->filestream, &uData) != UNZ_OK)
		err = UNZ_ERRNO;
	else if (err == UNZ_OK and uData != s->cur_file_info.uncompressed_size and (uFlags & 8) == 0)
		err = UNZ_BADZIPFILE;
...
*/
extern int32_t ZEXPORT unzOpenCurrentFile3(unzFile file,
	...
	pfile_in_zip_read_info =
	   static_cast<file_in_zip_read_info_s*>(malloc(sizeof(file_in_zip_read_info_s)));
	if (pfile_in_zip_read_info == nullptr)
		return UNZ_INTERNALERROR;

	pfile_in_zip_read_info->read_buffer = static_cast<char*>(malloc(UNZ_BUFSIZE));
	...
	if (s->cur_file_info.compression_method == Z_DEFLATED and (raw == 0)) {
...
extern int32_t ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, unsigned len) {
	...
	if (len > pfile_in_zip_read_info->rest_read_uncompressed and (pfile_in_zip_read_info->raw == 0))
	...
	while (pfile_in_zip_read_info->stream.avail_out > 0) {
		...
		if (pfile_in_zip_read_info->compression_method == 0 or (pfile_in_zip_read_info->raw != 0)) {
			uInt uDoCopy, i;
			...
			uDoCopy =
			   pfile_in_zip_read_info->stream.avail_out < pfile_in_zip_read_info->stream.avail_in ?
			      pfile_in_zip_read_info->stream.avail_out :
			      pfile_in_zip_read_info->stream.avail_in;
			...
			Bytef const* const bufBefore = pfile_in_zip_read_info->stream.next_out;
			...
			pfile_in_zip_read_info->crc32 =
			   crc32(pfile_in_zip_read_info->crc32, bufBefore, static_cast<uInt>(uOutThis));
...
extern int32_t ZEXPORT unzCloseCurrentFile(unzFile file) {
	...
	if (pfile_in_zip_read_info->rest_read_uncompressed == 0 and (pfile_in_zip_read_info->raw == 0)) {
	...
	if (pfile_in_zip_read_info->stream_initialized != 0u)
...
local linkedlist_datablock_internal* allocate_new_datablock() {
	linkedlist_datablock_internal* const ldi =
	   static_cast<linkedlist_datablock_internal*>(malloc(sizeof(linkedlist_datablock_internal)));
	if (ldi != nullptr) {
...
local int32_t add_data_in_datablock(linkedlist_data* const ll, void const* const buf, uLong len) {
	...
	while (len > 0) {
		...
		copy_this = ldi->avail_in_this_block < len ? static_cast<uInt>(ldi->avail_in_this_block) :
...
OF((zlib_filefunc_def const* pzlib_filefunc_def, voidpf filestream, uLong x, int32_t nbByte));
local int32_t ziplocal_putValue(zlib_filefunc_def const* const pzlib_filefunc_def,
                                voidpf filestream,
                                uLong x,
	...
	return ZWRITE(*pzlib_filefunc_def, filestream, buf, nbByte) != static_cast<uLong>(nbByte) ?
	          ZIP_ERRNO :
	          ZIP_OK;
}

local void ziplocal_putValue_inmemory OF((void* dest, uLong x, int32_t nbByte));
local void ziplocal_putValue_inmemory(void* const dest, uLong x, int32_t const nbByte) {
...
local uLong ziplocal_TmzDateToDosDate(tm_zip const* ptm, uLong) {
	...
	if (year > 1980)
		...
		year -= 80;
	return static_cast<uLong>((ptm->tm_mday + 32 * (ptm->tm_mon + 1) + 512 * year) << 16) |
	       (ptm->tm_sec / 2 + 32 * ptm->tm_min + 2048 * static_cast<uLong>(ptm->tm_hour));
}

/****************************************************************************/

local int32_t ziplocal_getByte OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                   voidpf filestream,
                                   int32_t* pi));

local int32_t ziplocal_getByte(zlib_filefunc_def const* const pzlib_filefunc_def,
                               voidpf filestream,
                               int32_t* const pi) {
	unsigned char c;
	const int32_t err = static_cast<int32_t>(ZREAD(*pzlib_filefunc_def, filestream, &c, 1));
...
/* ===========================================================================
   Reads a long in LSB order from the given gz_stream. Sets
*/
local int32_t ziplocal_getShort OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                    voidpf filestream,
                                    uLong* pX));

local int32_t ziplocal_getShort(zlib_filefunc_def const* const pzlib_filefunc_def,
                                voidpf filestream,
                                uLong* const pX) {
	uLong x;
	int32_t i = 0;
...
local int32_t ziplocal_getLong OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                   voidpf filestream,
                                   uLong* pX));

local int32_t ziplocal_getLong(zlib_filefunc_def const* const pzlib_filefunc_def,
                               voidpf filestream,
                               uLong* const pX) {
	uLong x;
	int32_t i = 0;
...
/*
  Locate the Central directory of a zipfile (at the end, just before
    the global comment)
*/
local uLong ziplocal_SearchCentralDir OF((zlib_filefunc_def const* pzlib_filefunc_def,
                                          voidpf filestream));

local uLong ziplocal_SearchCentralDir(zlib_filefunc_def const* const pzlib_filefunc_def,
                                      voidpf filestream) {
	unsigned char* buf;
	uLong uSizeFile;
	uLong uBackRead;
...
extern zipFile ZEXPORT zipOpen2(const char* pathname,
	...
	zip_internal* const zi = static_cast<zip_internal*>(malloc(sizeof(zip_internal)));
	if (zi == nullptr) {
...
//  now we add file in a zipfile
	...
	if (append == APPEND_STATUS_ADDINZIP) {
		...
		if  //  the signature, already checked
		   (ziplocal_getLong(&ziinit.z_filefunc, ziinit.filestream, &uL) != ZIP_OK)
			err = ZIP_ERRNO;

		if  //  number of this disk
		   (ziplocal_getShort(&ziinit.z_filefunc, ziinit.filestream, &number_disk) != ZIP_OK)
			err = ZIP_ERRNO;

		if  //  number of the disk with the start of the central directory
		   (ziplocal_getShort(&ziinit.z_filefunc, ziinit.filestream, &number_disk_with_CD) != ZIP_OK)
			err = ZIP_ERRNO;

		if  //  total number of entries in the central dir on this disk
		   (ziplocal_getShort(&ziinit.z_filefunc, ziinit.filestream, &number_entry) != ZIP_OK)
			err = ZIP_ERRNO;

		if  //  total number of entries in the central dir
		   (ziplocal_getShort(&ziinit.z_filefunc, ziinit.filestream, &number_entry_CD) != ZIP_OK)
			err = ZIP_ERRNO;

		if (number_entry_CD != number_entry or number_disk_with_CD != 0 or number_disk != 0)
			err = ZIP_BADZIPFILE;

		if  // size of the central directory
		   (ziplocal_getLong(&ziinit.z_filefunc, ziinit.filestream, &size_central_dir) != ZIP_OK)
			err = ZIP_ERRNO;
		...
		if  // zipfile comment length
		   (ziplocal_getShort(&ziinit.z_filefunc, ziinit.filestream, &size_comment) != ZIP_OK)
			err = ZIP_ERRNO;

		if (central_pos < offset_central_dir + size_central_dir and err == ZIP_OK)
			err = ZIP_BADZIPFILE;
			...
			while (size_central_dir_to_read > 0 and err == ZIP_OK) {
				...
				if (err == ZIP_OK)
					err = add_data_in_datablock(
					   &ziinit.central_dir, buf_read, static_cast<uLong>(read_this));
...
extern zipFile ZEXPORT zipOpen(char const* const pathname, int32_t const append) {
	return zipOpen2(pathname, append, nullptr, nullptr);
}

extern int32_t ZEXPORT zipOpenNewFileInZip3(zipFile file,
                                            char const* filename,
	...
	size_comment = comment != nullptr ? static_cast<uInt>(strlen(comment)) : 0;

	size_filename = static_cast<uInt>(strlen(filename));

	zi->ci.dosDate = zipfi == nullptr ? 0 : zipfi->dosDate != 0u ?
	                             zipfi->dosDate :
	...
	zi->ci.central_header = static_cast<char*>(malloc(static_cast<uInt>(zi->ci.size_centralheader)));

	ziplocal_putValue_inmemory(zi->ci.central_header + 0, static_cast<uLong>(CENTRALHEADERMAGIC), 4);
	// version info
	ziplocal_putValue_inmemory(zi->ci.central_header + 4, static_cast<uLong>(VERSIONMADEBY), 2);
	ziplocal_putValue_inmemory(zi->ci.central_header + 6, 20UL, 2);
	...
	ziplocal_putValue_inmemory(zi->ci.central_header + 16, 0UL, 4);  //  crc
	ziplocal_putValue_inmemory(zi->ci.central_header + 20, 0UL, 4);  //  compr size
	ziplocal_putValue_inmemory(zi->ci.central_header + 24, 0UL, 4);  //  uncompr size
	ziplocal_putValue_inmemory(zi->ci.central_header + 28, static_cast<uLong>(size_filename), 2);
	ziplocal_putValue_inmemory(
	   zi->ci.central_header + 30, static_cast<uLong>(size_extrafield_global), 2);
	ziplocal_putValue_inmemory(zi->ci.central_header + 32, static_cast<uLong>(size_comment), 2);
	ziplocal_putValue_inmemory(zi->ci.central_header + 34, 0UL, 2);  //  disk nm start

	ziplocal_putValue_inmemory(
	   zi->ci.central_header + 36, static_cast<uLong>(zipfi != nullptr ? zipfi->internal_fa : 0), 2);

	ziplocal_putValue_inmemory(
	   zi->ci.central_header + 38, static_cast<uLong>(zipfi != nullptr ? zipfi->external_fa : 0), 4);

	ziplocal_putValue_inmemory(
	   zi->ci.central_header + 42,
	   static_cast<uLong>(zi->ci.pos_local_header) - zi->add_position_when_writting_offset, 4);
	...
	err =
	   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(LOCALHEADERMAGIC), 4);
	...
	if (err == ZIP_OK)
		err =
		   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(zi->ci.method), 2);

	if (err == ZIP_OK)
		err =
		   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(zi->ci.dosDate), 4);

	//  crc 32, unknown
	if (err == ZIP_OK)
		err = ziplocal_putValue(&zi->z_filefunc, zi->filestream, 0UL, 4);
	...
	if (err == ZIP_OK)
		err =
		   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(size_filename), 2);

	if (err == ZIP_OK)
		err = ziplocal_putValue(
		   &zi->z_filefunc, zi->filestream, static_cast<uLong>(size_extrafield_local), 2);
	...
	if (err == ZIP_OK && zi->ci.method == Z_DEFLATED && (zi->ci.raw == 0)) {
...
extern int32_t ZEXPORT zipWriteInFileInZip(zipFile file,
	...
	zi->ci.stream.next_in = const_cast<Bytef*>(static_cast<const Bytef*>(buf));
	...
	while (err == ZIP_OK and zi->ci.stream.avail_in > 0) {
		...
		if (zi->ci.method == Z_DEFLATED and (zi->ci.raw == 0)) {
...
extern int32_t ZEXPORT zipCloseFileInZipRaw(zipFile file, uLong uncompressed_size, uLong crc32) {
	...
	if ((zi->ci.method == Z_DEFLATED) && (zi->ci.raw == 0)) {
	...
	if (zi->ci.stream.data_type == Z_ASCII)
		ziplocal_putValue_inmemory(zi->ci.central_header + 36, static_cast<uLong>(Z_ASCII), 2);
	ziplocal_putValue_inmemory(zi->ci.central_header + 24, uncompressed_size, 4);  //  uncompr size

	if (err == ZIP_OK)
		err = add_data_in_datablock(
		   &zi->central_dir, zi->ci.central_header, static_cast<uLong>(zi->ci.size_centralheader));
	free(zi->ci.central_header);

	if (err == ZIP_OK) {
		long cur_pos_inzip = ZTELL(zi->z_filefunc, zi->filestream);
		if (ZSEEK(
		       zi->z_filefunc, zi->filestream, zi->ci.pos_local_header + 14, ZLIB_FILEFUNC_SEEK_SET))
			err = ZIP_ERRNO;
		...
		if (ZSEEK(zi->z_filefunc, zi->filestream, cur_pos_inzip, ZLIB_FILEFUNC_SEEK_SET))
			err = ZIP_ERRNO;
	}
...
extern int32_t ZEXPORT zipClose(zipFile file, char const* const global_comment) {
	zip_internal* zi;
	int32_t err = 0;
	...
	size_global_comment = global_comment != nullptr ? static_cast<uInt>(strlen(global_comment)) : 0;
	...
	if (err == ZIP_OK)
		err =
		   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(ENDHEADERMAGIC), 4);
	...
	if (err == ZIP_OK)
		err = ziplocal_putValue(
		   &zi->z_filefunc, zi->filestream, static_cast<uLong>(zi->number_entry), 2);
	...
	if (err == ZIP_OK)
		err =
		   ziplocal_putValue(&zi->z_filefunc, zi->filestream, static_cast<uLong>(size_centraldir), 4);
	...
	if (err == ZIP_OK)
		err = ziplocal_putValue(
		   ...
		   static_cast<uLong>(centraldir_pos_inzip - zi->add_position_when_writting_offset), 4);

	//  zipfile comment length
	if (err == ZIP_OK)
		err = ziplocal_putValue(
		   &zi->z_filefunc, zi->filestream, static_cast<uLong>(size_global_comment), 2);

	if (err == ZIP_OK && size_global_comment > 0 and
	    ZWRITE(zi->z_filefunc, zi->filestream, global_comment, size_global_comment) !=
	       size_global_comment)
		err = ZIP_ERRNO;
...
voidpf ZCALLBACK fopen_file_func OF((voidpf opaque, const char* filename, int32_t mode));
...
long ZCALLBACK fseek_file_func OF((voidpf opaque, voidpf stream, uLong offset, int32_t origin));

int32_t ZCALLBACK fclose_file_func OF((voidpf opaque, voidpf stream));

int32_t ZCALLBACK ferror_file_func OF((voidpf opaque, voidpf stream));

voidpf ZCALLBACK fopen_file_func(voidpf, char const* const filename, int32_t const mode) {
	FILE* file = nullptr;
	char const* const mode_fopen =
	   (mode & ZLIB_FILEFUNC_MODE_READWRITEFILTER) == ZLIB_FILEFUNC_MODE_READ ?
	      "rb" :
	      (mode & ZLIB_FILEFUNC_MODE_EXISTING) != 0 ? "r+b" : (mode & ZLIB_FILEFUNC_MODE_CREATE) != 0 ? "wb" :
	                                                                                      nullptr;
...
uLong ZCALLBACK fread_file_func(voidpf, voidpf stream, void* const buf, uLong const size) {
	return fread(buf, 1, static_cast<size_t>(size), static_cast<FILE*>(stream));
}

uLong ZCALLBACK fwrite_file_func(voidpf, voidpf stream, void const* const buf, uLong const size) {
	return fwrite(buf, 1, static_cast<size_t>(size), static_cast<FILE*>(stream));
}
