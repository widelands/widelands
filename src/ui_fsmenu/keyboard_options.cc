...
KeyboardOptions::KeyboardOptions(Panel& parent)
   : UI::Window(&parent.get_topmost_forefather(),
	...
	auto create_tab = [this, add_key](const std::string& title,
	                                  ...
	                                  const KeyboardShortcut shortcut_end, UI::Box* b) {
	...
	const size_t fastplace_tab_index = tabs_.tabs().size();
	{
		...
		UI::Button* b1 = new UI::Button(box, "fp_clear", 0, 0, kButtonHeight, kButtonHeight,
		                                UI::ButtonStyle::kFsMenuSecondary, _("Clear"),
		                                _("Remove all Fastplace shortcuts"));
		box->add(b1, UI::Box::Resizing::kExpandBoth);
		box->add_inf_space();

		UI::Button* b2 = new UI::Button(
		   box, "fp_proposed", 0, 0, kButtonHeight, kButtonHeight, UI::ButtonStyle::kFsMenuSecondary,
		   _("Defaults"), _("Reset all Fastplace shortcuts to their proposed default values"));
		...
		b2->sigclicked.connect([all_keyboard_buttons, generate_title]() {
			set_fastplace_shortcuts_proposed();
			for (auto& pair : all_keyboard_buttons) {
				pair.second->set_title(generate_title(pair.first));
			}
		});
