...
std::string required_wl_version_and_sync_safety_string(std::shared_ptr<AddOns::AddOnInfo> info) {
	...
	if (!info->min_wl_version.empty() || !info->max_wl_version.empty()) {
		...
		if (info->max_wl_version.empty()) {
			str += bformat(_("Requires a Widelands version of at least %s.") ,
			        info->min_wl_version)
			          ;
		} else if (info->min_wl_version.empty()) {
			str +=
			   bformat(_("Requires a Widelands version of at most %s.") , info->max_wl_version)
			      ;
		} else {
			str +=
			   bformat(_("Requires a Widelands version of at least %1$s and at most %2$s.") ,
...
InstalledAddOnRow::InstalledAddOnRow(Panel* parent,
   ...
   : UI::Panel(parent,
     ...
     version_(this,
              ...
              bformat(_("%1$s+%2$u") , AddOns::version_to_string(info->version) ,
               info->i18n_version)
                 ,
              UI::Align::kCenter),
     txt_(this,
	...
	version_.set_tooltip(
	   /** TRANSLATORS: (MajorVersion)+(MinorVersion) */
	   bformat(_("Version: %1$s+%2$u") , AddOns::version_to_string(info->version) ,
	    info->i18n_version)
	      );
	set_can_focus(true);
	layout();
          ...
          bformat("<rt><p>%s</p><p>%s%s</p><p>%s</p></rt>")
           ...
           g_style_manager->font_style(UI::FontStyle::kItalic)
              .as_font_tag(info->author() == info->upload_username ?
                              ...
                              bformat(_("by %1$s (uploaded by %2$s)") , info->author() ,
	...
	install_.sigclicked.connect([ctrl, this]() {
		...
		if (!info_->verified || !(SDL_GetModState() & KMOD_CTRL)) {
			UI::WLMessageBox w(
			   ...
			   safe_richtext_message(
			      bformat(_("Are you certain that you want to install this add-on?\n\n"
			                       "%1$s\n"
			                       "by %2$s\n"
			                       "%3$s\n"
			                       "Version %4$s\n"
	...
	upgrade_.sigclicked.connect([this, ctrl, info, installed_version]() {
		if (!info->verified || !(SDL_GetModState() & KMOD_CTRL)) {
			UI::WLMessageBox w(
			   ...
			   safe_richtext_message(
			      bformat(_("Are you certain that you want to upgrade this add-on?\n\n"
			                       "%1$s\n"
			                       "by %2$s\n"
			                       "%3$s\n"
