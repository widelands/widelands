...
AddOnsCtrl::AddOnsCtrl(MainMenu& fsmm, UI::UniqueWindow::Registry& reg)
   : UI::UniqueWindow(&fsmm,
	   ...
	   new UI::MultilineTextarea(
	      ...
	      bformat("<rt><p>%1$s</p></rt>" ,
	       g_style_manager->font_style(UI::FontStyle::kFsMenuInfoPanelParagraph)
	          .as_font_tag(
	             bformat(_("For more information regarding how to develop and package your "
	...
	dev_box_.add(
	   new UI::MultilineTextarea(
	      &dev_box_, 0, 0, 100, 100, UI::PanelStyle::kFsMenu,
	      bformat("<rt><p>%s</p><p>%s</p><p>%s</p><p>%s</p></rt>" ,
	       g_style_manager->font_style(UI::FontStyle::kFsMenuInfoPanelParagraph)
	          .as_font_tag(
...
void AddOnsCtrl::update_login_button(UI::Button* b) {
	if (username_.empty()) {
		...
		if (b) {
			b->set_title(
			   bformat(net().is_admin() ? _("Logged in as %s (admin)") : _("Logged in as %s") ,
			    username_)
			      );
		...
		for (const std::string& requirement : addon->first->requirements) {
			...
			if (search_result == AddOns::g_addons.end()) {
				...
				if (!search_result->second) {
					warn_requirements.push_back(
					   bformat(_("· ‘%1$s’ requires ‘%2$s’ which is disabled") ,
			...
			for (const std::string& previous_requirement : addon->first->requirements) {
				...
				if (too_late && AddOns::order_matters(prev->category, next->category)) {
					warn_requirements.push_back(
					   bformat(_("· ‘%1$s’ requires first ‘%2$s’ and then ‘%3$s’, but they are "
