...
void CampaignDetails::update(const CampaignData& campaigndata) {
	name_label_.set_text(
		(boost::format("<rt>%s%s</rt>") %
		 as_header(_("Campaign Name:"), true) %
		 as_content(campaigndata.name))
			.str());

	std::string description =
		(boost::format("%s%s") % as_header("Tribe:") % as_content(campaigndata.tribename))
			.str();

	description =
		(boost::format("%s%s") % description % as_header(_("Difficulty:"))).str();
	description =
		(boost::format("%s%s") % description % as_content(campaigndata.difficulty_description)).str();

	description =
		(boost::format("%s%s") % description % as_header(_("Story:"))).str();
	description =
		(boost::format("%s%s") % description % as_content(campaigndata.description)).str();
