...
void Game_Server_Connection::send(Game_Server_Protocol_Packet* packet) {
	uint16_t const id    = packet->get_id();
	uint32_t const index = m_last_packet_index++;
	...
	m_pending_packets.insert
		(std::pair<uint32_t, Game_Server_Protocol_Packet*>(index, packet));
...
void Game_Server_Connection::handle_data() {
	...
	if (SDLNet_CheckSockets(m_socketset, 0) > 0) {
		...
		//  get the header
		uint16_t const id    = buf.get_16();
		uint32_t const index = buf.get_32();
		uint16_t const flags = buf.get_16();

		if (IS_ANSWER(flags)) {
			if (not m_pending_packets.count(index)) {
				log
					("Game_Server_Connection: WARNING Unknown response packet with "
					 "id %i, dropped\n",
					 index);
				return;
			}
			...
			if (pp->get_id() != id)  {
				log
					("Game_Server_Connection: WARNING Response packet with wrong "
					 "id (has: %i, should: %i), dropped\n",
					 pp->get_id(), id);
				return;
			}
...
void Game_Server_Connection::set_room(const char* room) {
	...
	Game_Server_Protocol_Packet_GetRoomInfo & gri =
		*new Game_Server_Protocol_Packet_GetRoomInfo(room);
	send(&gri);
}
