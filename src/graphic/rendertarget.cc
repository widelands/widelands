...
void RenderTarget::blit(const Vector2i& dst,
	...
	Recti source_rect(Vector2i(0, 0), image->width(), image->height());
	Recti destination_rect(destination_point.x, destination_point.y, source_rect.w, source_rect.h);
...
void RenderTarget::blitrect(const Vector2i& dst,
	...
	Recti source_rect(gsrcrc.x, gsrcrc.y, std::min<int32_t>(image->width() - gsrcrc.x, gsrcrc.w),
	                 std::min<int32_t>(image->height() - gsrcrc.y, gsrcrc.h));
	Recti destination_rect(dst.x, dst.y, source_rect.w, source_rect.h);
...
void RenderTarget::blitrect_scale(Recti destination_rect,
                                  const Image* image,
                                  Recti source_rect,
                                  const float opacity,
...
void RenderTarget::blitrect_scale_monochrome(Recti destination_rect,
                                             const Image* image,
                                             Recti source_rect,
	...
	if (clip(r)) {
		...
		while (ty < r.h) {
			...
			while (tx < r.w) {
				...
				const Recti dst_rect(r.x + tx, r.y + ty, srcrc.w, srcrc.h);
...
void RenderTarget::blit_animation(const Vector2f& dst,
	...
	do_blit_animation(dst, zoom, anim, time, nullptr, Recti(Vector2i(0, 0), anim.width(), anim.height()));
}
 ...
 */
bool RenderTarget::clip(Recti& r) const {
	r.x += offset_.x;
	r.y += offset_.y;
 ...
 */
bool RenderTarget::to_surface_geometry(Recti* destination_rect, Recti* source_rect) const {
