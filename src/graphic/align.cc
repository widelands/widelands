...
Rectf fit_image_crop(float img_w, float img_h, float available_w, float available_h) {
...
Rectf fit_image_nocrop(float img_w, float img_h, float available_w, float available_h) {
	if (img_w < available_w && img_h < available_h) {
		return Rectf((available_w - img_w) / 2.f, (available_h - img_h) / 2.f, img_w, img_h);
	}
	const float new_h = img_h * available_w / img_w;
