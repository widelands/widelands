	...
	void advance_line()
	{
		...
		if (elements.empty()) {
			...
			if (text_y < rti.height + images_height) {
				if ((mirror_alignment(richtext->get_image_align()) & UI::Align::Align_Horizontal) == UI::Align::Align_Right) {
			...
			switch (mirror_alignment(richtext->get_text_align()) & UI::Align::Align_Horizontal) {
...
void RichText::parse(const std::string & rtext)
{
	...
	for (text.richtext = blocks.begin(); text.richtext != blocks.end(); ++text.richtext) {
		...
		if ((text.richtext->get_image_align() & UI::Align::Align_Horizontal) == UI::Align::Align_HCenter)
			imagealigndelta = (int32_t(m->width) - int32_t(text.images_width)) / 2;
		else if ((mirror_alignment(text.richtext->get_image_align()) & UI::Align::Align_Horizontal) == UI::Align::Align_Right)
