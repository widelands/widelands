 ...
 */
void SfxCues::read(const string & directory, Section & section)
{
...
public:
	BlitsAnimation(const string & directory, Section & section);
...
BlitsAnimation::BlitsAnimation(const string & directory, Section & section) :
	frametime_(FRAME_LENGTH)
{
	hash_ = directory + section.get_name();

	sfx_cues_.read(directory, section);

	const int32_t fps = section.get_int("fps");
	if (fps < 0)
	...
	// Load the graphis
	string spritemap = directory + section.get_safe_string("spritemap");
	...
	// Parse frames
	const int32_t nrframes = section.get_int("nrframes");
	if (nrframes <= 0)
	...
	for (int32_t framenr = 0; framenr < nrframes; ++framenr) {
		vector<Blit> blits;
		string framedata = section.get_safe_string(boost::lexical_cast<string>(framenr));
...
public:
	...
	PackedAnimation(const string& directory, Section & section);
...
PackedAnimation::PackedAnimation(const string& directory, Section& section)
		: width_(0), height_(0), nr_frames_(0), frametime_(FRAME_LENGTH), image_(NULL), pcmask_(NULL) {
	hash_ = directory + section.get_name();

	sfx_cues.read(directory, section);
	...
	// Load the graphis
	string pic_fn = directory + section.get_safe_string("pics");
	...
	// Parse dimensions.
	Point p;
	parse_point(section.get_string("dimensions"), &p);
	width_ = p.x;
	height_ = p.y;

	// Parse base_offset.
	parse_point(section.get_string("base_offset"), &base_offset_);

	// Parse regions
	NumberGlob glob("region_??");
	string region_name;
	while (glob.next(&region_name)) {
		string value = section.get_string(region_name.c_str(), "");
		if (value.empty())
			break;
