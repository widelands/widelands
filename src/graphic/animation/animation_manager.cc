...
uint32_t
AnimationManager::load(const LuaTable& table, const std::string& basename, const std::string& animation_directory, Animation::Type type) {
	switch (type) {
	case Animation::Type::kFiles:
		animations_.push_back(std::unique_ptr<Animation>(new NonPackedAnimation(table, basename, animation_directory)));
		break;
	case Animation::Type::kSpritesheet:
		animations_.push_back(std::unique_ptr<Animation>(new SpriteSheetAnimation(table, basename, animation_directory)));
	}
	return animations_.size();
}
uint32_t AnimationManager::load(const std::string& map_object_name,
                                const LuaTable& table,
                                const std::string& basename, const std::string& animation_directory,
                                Animation::Type type) {
