   ...
   const Widelands::Player* player) {
	...
	for (size_t current_index = 0; current_index < fields_to_draw.size(); ++current_index) {
		...
		if (field.bln_index != FieldsToDraw::kInvalidIndex) {
			const Widelands::DescriptionIndex terrain_d = player ?
					player->fields()[map->get_index(field.fcoords)].terrains.d :
					field.fcoords.field->terrain_d();
			const Widelands::DescriptionIndex terrain_r = player ?
					player->fields()[map->get_index(field.fcoords)].terrains.r :
			...
			const Widelands::DescriptionIndex terrain_dd = player ?
					player->fields()[map->get_index(map->bl_n(field.fcoords))].terrains.r :
			...
			maybe_add_dithering_triangle(gametime, terrains, fields_to_draw, field.bln_index,
			                             ...
			                             terrain_d, terrain_dd);

			if (field.ln_index != FieldsToDraw::kInvalidIndex) {
				const Widelands::DescriptionIndex terrain_l = player ?
						player->fields()[map->get_index(map->l_n(field.fcoords))].terrains.r :
		...
		if (field.rn_index != FieldsToDraw::kInvalidIndex) {
			...
			maybe_add_dithering_triangle(
			   ...
			   terrain_r, terrain_d);
			const Widelands::DescriptionIndex terrain_rr = player ?
					player->fields()[map->get_index(map->r_n(field.fcoords))].terrains.d :
			...
			maybe_add_dithering_triangle(gametime, terrains, fields_to_draw, field.brn_index,
			                             ...
			                             terrain_r, terrain_rr);

			if (field.trn_index != FieldsToDraw::kInvalidIndex) {
				const Widelands::DescriptionIndex terrain_u = player ?
						player->fields()[map->get_index(map->tr_n(field.fcoords))].terrains.d :
