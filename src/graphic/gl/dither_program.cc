...
void DitherProgram::draw(
   ...
   const Widelands::Player* player) {
	...
	for (size_t current_index = 0; current_index < fields_to_draw.size(); ++current_index) {
		...
		const Widelands::Map* map = (player != nullptr) && !player->see_all() ? &player->egbase().map() : nullptr;
		...
		if (field.bln_index != FieldsToDraw::kInvalidIndex) {
			...
			const Widelands::DescriptionIndex terrain_dd =
			   map != nullptr ? player->fields()[map->get_index(map->bl_n(field.fcoords))].terrains.load().r :
			...
			if (field.ln_index != FieldsToDraw::kInvalidIndex) {
				const Widelands::DescriptionIndex terrain_l =
				   map != nullptr ? player->fields()[map->get_index(map->l_n(field.fcoords))].terrains.load().r :
		...
		if (field.rn_index != FieldsToDraw::kInvalidIndex) {
			...
			const Widelands::DescriptionIndex terrain_rr =
			   map != nullptr ? player->fields()[map->get_index(map->r_n(field.fcoords))].terrains.load().d :
			...
			if (field.trn_index != FieldsToDraw::kInvalidIndex) {
				const Widelands::DescriptionIndex terrain_u =
				   map != nullptr ? player->fields()[map->get_index(map->tr_n(field.fcoords))].terrains.load().d :
