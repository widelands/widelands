...
std::string as_tooltip_text_with_hotkey(const std::string& text, const std::string& hotkey, const UI::PanelStyle style) {
	static boost::format f("<rt><p>%s %s</p></rt>");
	f % g_style_manager->font_style(style == UI::PanelStyle::kWui ? UI::FontStyle::kWuiTooltip : UI::FontStyle::kFsTooltip).as_font_tag(text);
	f % g_style_manager->font_style(style == UI::PanelStyle::kWui ?
			UI::FontStyle::kWuiTooltipHotkey : UI::FontStyle::kFsTooltipHotkey).as_font_tag("(" + hotkey + ")");
	return f.str();
}
