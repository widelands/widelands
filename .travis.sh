...
build)
   if [ "$TRAVIS_OS_NAME" = linux ]; then
      ...
      cmake .. -DCMAKE_BUILD_TYPE:STRING=$BUILD_TYPE -DOPTION_BUILD_TRANSLATIONS=$BUILD_TRANSLATIONS -DOPTION_BUILD_WEBSITE_TOOLS=$BUILD_WEBSITE_TOOLS -DOPTION_ASAN="OFF" -DOPTION_BUILD_CODECHECK="OFF" -DLDFLAGS="-L/usr/local/opt/gettext/lib" -DCXXFLAGS="-I/usr/local/opt/gettext/include"
   fi
   # Do the actual build.
   make -k -j3
